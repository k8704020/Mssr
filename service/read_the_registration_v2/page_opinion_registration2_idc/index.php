...
$funcs=array(
   ...
   //---------------------------------------------------
    ...
    //---------------------------------------------------
		...
		$school_code_results=db_result($conn_type='pdo',$conn_user,$sql,array(),$arry_conn_user);
		$_SESSION['school_code']=$school_code_results[0]['school_code'];
		if(isset($_SESSION['school_code']))$school_code=$_SESSION['school_code'];
...
<body>
	...
	<div id="cover" style="position:absolute; top:-8px; left:-8px; display:none;">
    	<div onClick="" style="position:relative; top:0px; left:0px; height:495px; width:950px; cursor:wait; background-color:#000; opacity:0.7; z-index:9999"></div>
    ...
    <div style="position:absolute; top:0px; left:0px; width:955px; height:495px; background-color:#56ffd6; z-index:;"></div>
    <!--書籍資料顯示-->
    <!--標題開頭-->
    <p id="tittle"  style="position:absolute; top:0px; left:31%;font-size: 36px; font-weight: 600;" class="box_ling">
    &nbsp;&nbsp;&nbsp;&nbsp;回答閱讀書籍相關問題&nbsp;&nbsp;&nbsp;&nbsp;</p>
    ...
    <div id="out_b" style="position:absolute; top:10px; right: 0px;  height:46px; background-color:#F8C967"></div>
    <!-- 改過 -->
    <a id="out" class="btn_close" onClick="go_out()" style="position:absolute; top:5px; left:868px; cursor:pointer; display:none"></a>
	...
	<!--確認頁框--><a id="btn_help_page_n" class="btn_help_page_n"  style="position:absolute; top:435px; left:18px; display:none;"></a>
	<!-- 填寫頁數 -->
    <!-- <a id="btn_help_page" class="btn_help_page" onClick="open_vate_page()" style="position:absolute; top:435px; left:18px; cursor:pointer;display:none;"></a> -->
...
<script>
	...
	var about_book_sid=localStorage["about_book_sid"];
	var about_book_name=localStorage["about_book_name"];
	var about_book_author=localStorage["about_book_author"];
	var about_book_page_count=localStorage["about_book_page_count"];
	var about_book_publisher=localStorage["about_book_publisher"];
	var book_borrow_sid=localStorage["book_borrow_sid"];
	var about_book_src=localStorage["about_book_src"];
	...
	function set_quest_v2()
	{
		...
		if(quest_list[questpage]["type"] == "radio")
		{
			...
			{
				if(re_answer[questpage] != i)window.document.getElementById("answer").innerHTML += '<input  type="radio" style="width:24px;height:24px;position:relative; top:4px;" name="answer" id="answer'+i+'" >'+'<label for="answer'+i+'">'+quest_list[questpage]["answer"][i]+"<label><BR>";
				else window.document.getElementById("answer").innerHTML += '<input  type="radio" style="width:24px;height:24px; position:relative; top:4px;" name="answer" id="answer'+i+'" checked>'+'<label for="answer'+i+'">'+quest_list[questpage]["answer"][i]+'<label><BR>';
			}
		}else if(quest_list[questpage]["type"] == "number")
		{
...
//---------------------------------------------------
	...
	{
		echo("sp_7_set_quset:讀取頁數填寫資訊+++"+about_book_sid);
		cover("讀取中")
			...
			}).success(function (data)
			{
				...
				}else
				{
					...
					else
					{
						...
						// if(window.parent.tittle =="st")
						// window.parent.set_page("page_opinion_menu");
						document.location.href="../../bookstore_v2/page_opinion_menu_idc/index.php";
					}
	...
	function show_book()
	{
		...
		// if(window.parent.tittle =="st")
		// {
			window.document.getElementById("out_b").style.right = "0px";
		...
		// console.log(localStorage["about_book_name"]);
		// console.log(localStorage["about_book_name"]);
