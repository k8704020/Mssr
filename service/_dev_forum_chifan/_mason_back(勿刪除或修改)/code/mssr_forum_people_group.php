        ...
        //-----------------------------------------------
	        ...
	        //-----------------------------------------------
	        	...
	        	//-----------------------------------------------
					$query_sql="
						SELECT
							`mssr_forum`.`forum_name`, `mssr_forum`.`forum_id`, `mssr_forum`.`forum_state`, `mssr_user_forum`.`forum_id`, `mssr_user_forum`.`user_id`
						FROM
							`mssr_forum`
						INNER JOIN
							`mssr_user_forum`
...
//				die();
    ...
    //---------------------------------------------------
		...
		if(count($arrys_result_forum)!==0){
			...
			$numrow=count($arrys_result_forum);
			...
			$arrys_result_forum=db_result($conn_type='pdo',$conn_mssr,$query_sql,array($sinx-1,$psize),$arry_conn_mssr);
...
<header>
	...
	<nav>
    	<ul>
            ...
            href="javascript:void(0);" style="background-image:url(image/icon_circle.png); background-repeat:no-repeat; background-position:center">聊書小組</a></li>
...
<section class="course_2">
	...
	<?php
		if(empty($arrys_result_forum)){
			...
			for($i=0;$i<count($arrys_result_forum);$i++){
				$forum_name 	= trim($arrys_result_forum[$i]['forum_name']);
				$forum_id 		= (int)$arrys_result_forum[$i]['forum_id'];
				...
				<figure class="figure_group">
					<a onclick="action_log('inc/add_action_forum_log/code.php','p6',<?php echo $_SESSION["uid"];?>,<?php echo $user_id;?>,0,'','',<?php echo $forum_id;?>,0,0,0,'mssr_forum_group_discussion.php?forum_id=<?php echo $forum_id?>');void(0);"
                    href="javascript:void(0);"><img src="<?php echo $rs_forumpic_root?>" alt=""  /></a>
					<figcaption class="figcaption_group"><?php echo $forum_name?></figcaption>
				</figure>
			<?php }?>
    	<?php }?>
</section>
