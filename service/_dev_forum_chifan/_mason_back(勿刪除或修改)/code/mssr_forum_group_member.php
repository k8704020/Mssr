        ...
        //-----------------------------------------------
	        ...
	        //-----------------------------------------------
	        	...
	        	//-----------------------------------------------
					...
					//$numrow=count($arrys_result_member);
				//-----------------------------------------------
	        	//SQL-討論區資訊
	        	//-----------------------------------------------
					$sql="
	        	...
	        	//-----------------------------------------------
					...
					//$numrow_aside=count($arrys_result_aside);
				//echo "<pre>";
//                print_r($arrys_group_stud_check);
//                echo "</pre>";
//				die();
	//---------------------------------------------------
    //資料,設定
    ...
    //---------------------------------------------------
		...
		if(count($arrys_result_member)!==0){
			if(isset($_GET['psize'])){
...
<script>
    ...
    //加為好友
        ...
        var url ='';
        var page=str_repeat('../',0)+'add/add_groupA.php';
        var arg ={
            'sess_uid':sess_uid,
            'forum_id' :forum_id,
            'psize'   :psize,
...
<header>
    ...
    <img id="open_mssr_input_box" src="image/article_publish.png" alt="" width="50" height="50"
     ...
     <!----------加入社團按鈕---------->
   <?php
	if(count($arrys_group_stud_check)==0){?>
    	<img id="open_mssr_forum_group_add_friend" src="image/add_group.png" alt="" width="50" height="50"
            style='position:relative;z-index:99;';
            onmouseover="mouse_over(this);void(0);"
            onclick="add_group();void(0);" />
	<?php }?>
	...
	<nav>
    	<ul>
            ...
            <li><a onclick="action_log('inc/add_action_forum_log/code.php','g4',<?php echo $_SESSION["uid"];?>,0,0,'','',<?php echo $forum_id?>,0,0,0,'mssr_forum_group_member.php?forum_id=<?php echo $forum_id?>');void(0);"
                href="javascript:void(0);" style="background-image:url(image/icon_circle.png); background-repeat:no-repeat; background-position:center">聊書小組成員</a></li>
        </ul>
    </nav>
</header>
