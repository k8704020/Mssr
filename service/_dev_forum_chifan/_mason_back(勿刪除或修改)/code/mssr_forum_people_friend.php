...
//-------------------------------------------------------
    ...
    //---------------------------------------------------
		...
		$sinx  =0;  //值域起始值
		$einx  =0;  //值域終止值
		if(count($arrys_result_friend)!==0){
			if(isset($_GET['psize'])){
			...
			$numrow=count($arrys_result_friend);
			...
			$arrys_result_friend=db_result($conn_type='pdo',$conn_mssr,$query_sql,array($sinx-1,$psize),$arry_conn_mssr);
		}else{}
...
<header>
	...
	<nav>
    	<ul>
            ...
            href="javascript:void(0);" style="background-image:url(image/icon_circle.png); background-repeat:no-repeat; background-position:center">朋友</a></li>
        </ul>
    </nav>
</header>
...
<div class="table_page">
	<?php if(count($arrys_result_friend)!==0):?>
	...
	<?php
		if(empty($arrys_result_friend)){
			...
			for($i=0; $i<count($arrys_result_friend); $i++){
				...
				$user_id 			= (int)$arrys_result_friend[$i]['user_id'];
				$friend_id 			= (int)$arrys_result_friend[$i]['friend_id'];
                ...
                <?php
                	//處理
					if($friend_sex==1){?>
                        <a onclick="action_log('inc/add_action_forum_log/code.php','p7',<?php echo $_SESSION["uid"];?>,<?php echo $rs_user_id;?>,<?php echo $rs_friend_id;?>,'','',0,0,0,0,'mssr_forum_people_shelf.php?user_id=<?php echo $rs_friend_id?>');void(0);"
                        href="javascript:void(0);"><img src="image/boy.jpg" alt="<?php echo $friend_name?>" width="100px" height="100px" /></a>
					<?php }else{?>
						<a onclick="action_log('inc/add_action_forum_log/code.php','p7',<?php echo $_SESSION["uid"];?>,<?php echo $rs_user_id;?>,<?php echo $rs_friend_id;?>,'','',0,0,0,0,'mssr_forum_people_shelf.php?user_id=<?php echo $rs_friend_id?>');void(0);"
                        href="javascript:void(0);"><img src="image/girl.jpg" alt="<?php echo $friend_name?>" width="100px" height="100px" /></a>
					<?php }?>

					<figcaption><?php echo $friend_name?></figcaption>
				</figure>
   			<?php }?>
    	<?php }?>
</section>
