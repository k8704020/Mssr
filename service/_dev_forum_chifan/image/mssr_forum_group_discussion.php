...
<title><?php echo $title?></title>
	...
	<script>
		...
		function open_move_best_article(choose_article_id, user_id, forum_id) {
			...
			$('#move_best_ui_leave').click(function() {
				$.unblockUI();
				return false;
			});
			...
			?>
					...
					<img src="<?php echo $rs_bookpic_root ?>" alt="<?php echo $book_name?>" width="50px" height="60px" />
						<figcaption class="figcaption_book">
							<input type="radio" name="select_book_sid" value="<?php echo $book_sid?>">
...
<!----------block ui div(發表文章-輸入發文內容)---------->
<div id="mssr_article_input_box"  style="display:none; cursor: default;">
    <form id="article_input_form" name="article_input_form" action="mssr_forum_group_discussion_A.php" method="post">
        ...
        <h3 id="mssr_input_box_name" style="color:#660000;font-family:微軟正黑體;align=center;"><B>輸入發文內容與標題</B></h3>
		<input type="hidden" value="無" name="input_article_book_sid" id="input_article_book_sid">
		<img src="image/4.png" style="top:10px;	left:430px;"> 
        <p id="mssr_input_box_title">標題：<input  id="haha" type="text"  name="mssr_input_box_name_title" size="40" maxlength="40"/></p>


		<span id="mssr_input_box_content">內容：</span>
		<div id="mssr_input_box_content_text">
			<font color=red>請選擇右邊要欲輸入的內容</font>
		</div>
		...
		<?php
			//判斷是不是小說類
			$has_fiction = TRUE;
			$book_code_FN =	substr($book_code,0,1);
			if($book_code_FN != "8"){
				$has_fiction =false;
			}
			if($has_fiction){
		?>
		<div id="mssr_input_box_scaffolding_radio" class="btn-group" data-toggle="buttons">
			<label class="btn btn-info active" onclick="scaffolding_change_FL();">
				<input id="Radio1" type="radio" name="radio"/> 我想要分享
			</label>
			<label class="btn btn-info" onclick="scaffolding_change_FA();">
				<input type="radio" name="options" id="option2" onclick="scaffolding_changeFA()"> 我想要問
			</label>
		</div>


		<!--scaffolding start-->
		<div id="scaffolding_fiction">
			<select id="select_input_type" class="form-control"  name="type" style="display:none"
				onchange="scaffolding_text(this.options[this.options.selectedIndex].value)"  style="width:290px">
				<option value="11">你覺得這個故事在說什麼？</option>
				<option value="12">你有跟故事有類似的經驗嗎？</option>
				<option value="13">你看完這個故事的感覺是什麼？</option>
				<option value="14">你對這個故事內容有什麼看法？</option>
			</select>

			<textarea name="forum_id" style="display:none" cols="3" rows="8"><?php echo (int)$_GET["forum_id"]?></textarea>

			<div id="select_input_radio" style="display:none">
				<input id="r1"  class="mssr_input_box_radio" type="radio" name="article_refer_code" value="11a" onclick="article_input_text('r1')" />
					<p id="p1" class="mssr_input_box_radio_text" style="width:280px; height:25px;" >這本書的情節在說……</p>
				<input id="r2"  class="mssr_input_box_radio" type="radio" name="article_refer_code" value="11b" onclick="article_input_text('r2')" />
					<p id="p2" class="mssr_input_box_radio_text" style="width:280px; height:25px;">這是……的故事，主角是……，一開始……，後來……</p>
				<input id="r3"  class="mssr_input_box_radio" type="radio" name="article_refer_code" value="11c" onclick="article_input_text('r3')"/>
					<p id="p3" class="mssr_input_box_radio_text" style="width:280px; height:25px;">這本書的情節背景是……</p>
				<input id="r0"  class="mssr_input_box_radio" type="radio" name="article_refer_code" value="0" onclick="article_input_text('0')"/>
						<p id="p0" class="mssr_input_box_radio_text" style="width:280px; height:25px;">其他</p>
			</div>

		</div>

		<!--scaffolding_learn end-->
		<?php
			}else{

		?>
			<div id="mssr_input_box_scaffolding_NF_radio" class="btn-group" data-toggle="buttons">
				<label class="btn btn-info active" onclick="scaffolding_change_NFL();">
					<input id="Radio1" type="radio" name="radio"/> 我想要分享
				</label>
				<label class="btn btn-info" onclick="scaffolding_change_FA();">
					<input type="radio" name="options" id="option2" onclick="scaffolding_change_NFA()"> 我想要問
				</label>
			</div>

			<div id="scaffolding_nonfiction">
				<select id="select_input_type" class="form-control"  name="type"
					onchange="scaffolding_text(this.options[this.options.selectedIndex].value)"  style="width:290px">

					<option value="31">你覺得這本書內容的重點是什麼？</option>
					<option value="32">你有書中相關的知識想要分享嗎？</option>
					<option value="33">關於書中的內容你有什麼想要評論？</option>

				</select>

			<textarea name="forum_id"  style="display:none" cols="3" rows="8"><?php echo (int)$_GET["forum_id"]?></textarea>
				<input id="r1"  class="mssr_input_box_radio" type="radio" name="article_refer_code" value="11a" onclick="article_input_text('r1')"/>
					<p id="p1" class="mssr_input_box_radio_text" style="width:280px; height:25px;">這本書在說跟……有關的知識，例如……</p>
				<BR>
				<input id="r2"  class="mssr_input_box_radio" type="radio" name="article_refer_code" value="11b" onclick="article_input_text('r2')"/>
					<p id="p2" class="mssr_input_box_radio_text" style="width:280px; height:25px;">我覺得這本書的重點概念是……，因為……</p>
				<BR>
				<input id="r3"  class="mssr_input_box_radio" type="radio" name="article_refer_code" value="11c" onclick="article_input_text('r3')"/>
					<p id="p3" class="mssr_input_box_radio_text" style="width:280px; height:25px;">這本書提到……的知識，重點是……</p>
...
<div id="mark_input_box"  style="display:none; cursor: default;">
