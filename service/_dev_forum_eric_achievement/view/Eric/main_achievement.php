...
<head>
	...
	//---------------------------------------------------------
		...
		$.ajax({//抓取學生的表現，傳給function產生視圖
			...
			"success": function(data){
				...
				sex              = data["sex"];
				dission_topic	 = data["dission_topic"];
				dission_topic_id = data["dission_topic_id"];
	...
	//---------------------------------------------------------			
		 function get_data(i,j){//(i:成就類型；j:div的名稱)
			...
			if(type==0){//成就總覽
				var add ="";//產生成就總體>總個數
					add += "<div style='margin-left:2%;margin-right:2%;'>";
					add += "<table class='TB'>";
					add += "<tr><td style='background-color:#FFFFBB;'><img src='img/mission_icon/information.png'> &nbsp;需要在以下的任務版發文才可以得到分數！</td></tr>";
					add += "<tr>";
					for(var k=1; k<=topic_number; k++){						
						add_s += k+". "+dission_topic[k-1]+" ─ [<a href='../_dev_group_mission.php?get_from=3&group_task_id="+dission_topic_id[k-1]+"' target='_blank'>點我前往觀看</a>]</br>";
					}
					add += "<td>"+add_s+"</td>";
					add += "</tr>";
					add += "</table>";
