    ...
    //---------------------------------------------------
		...
		if(isset($arrys_sess_login_info[0]['uid'])){
			...
			//-------------------------------------------------------------------
				...
				//-------------------------------------------------------------------
				$dicussion_cno = 0;//討論人數
				$sql="
                    SELECT sum(A.dicussion_cno) AS dicussion_cnos
						FROM
					(SELECT count(`reply_id`) as dicussion_cno
						FROM `mssr_forum`.`mssr_forum_reply`
					WHERE `article_id`={$article_id}
					group by `user_id`) AS A
				";
				$get_dicussion_cno =db_result($conn_type='pdo',$conn_mssr,$sql,array(),$arry_conn_mssr);
				if(isset($get_dicussion_cno[0]['dicussion_cnos'])){
					$dicussion_cno = $get_dicussion_cno[0]['dicussion_cnos'];
				}
				//-------------------------------------------------------------------
				...
				echo json_encode(array("step_number"=>$step_number,"article_id"=>$article_id,"article_title"=>$article_title,"end_time"=>$end_time,"step_content"=>$step_content,"score_content"=>$score_content,"like_cno"=>$article_like_cno+$article_reply_like_cno,"article_reply_cno"=>$article_reply_cno,"expect_score"=>$expect_score,"dicussion_cno"=>$dicussion_cno));
...
/* 				$get_name = array();//暫存好友們的名字
			...
			//--------------------------------------------------
				...
				"friend_list"			=> $friend_select_list,
				"get_name"				=> $get_name));
			}
		}
