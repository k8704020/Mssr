<?php
	...
	if(isset($arrys_sess_login_info[0]['uid'])){
		...
		$sql="
			SELECT
				count(a1.`article_id`) as like_cno
		...
		UNION ALL
			SELECT
				count(r1.`reply_id`) as like_cno
			FROM `mssr_forum`.`mssr_forum_reply_like_log` as r1
				inner join `mssr_forum`.`mssr_forum_reply` as r2 on (r1.`reply_id` = r2.`reply_id`)
			WHERE 1=1
				AND r1.`user_id`={$uid}
				AND r2.`reply_from`=3
			group by r1.`user_id`
		...
		$sql="
			...
			FROM (
				SELECT
					`user_id`
				FROM `mssr_forum`.`mssr_forum_article` as a
				inner join `mssr_forum`.`mssr_forum_article_eagle_rev` as ae on (a.`article_id` = ae.`article_id`)
				WHERE 1=1
					AND `user_id`={$uid}
					AND `eagle_code` !=0
					AND `article_from`=3
		...
		";
		$achieve_data =db_result($conn_type='pdo',$conn_mssr,$sql,array(),$arry_conn_mssr);
		if(isset($achieve_data[0]['cno'])){$cno[4] =(int)$achieve_data[0]['cno'];}
		//請求推薦
		$cno[5] = 0;
		$sql="
		...
		for( $x=1; $x<=6; $x++){
			...
			//-----------------------------------------------
			$dission_topic    = array();
			$dission_topic_id = array();
			$sql="
				SELECT *
				FROM `mssr_forum`.`dev_group_mission`
				WHERE 1=1
					AND `group_task_id` not in(2,3,4)
			";
			$get_topic =db_result($conn_type='pdo',$conn_mssr,$sql,array(),$arry_conn_mssr);
			foreach($get_topic as $key => $get_topic){				
				$dission_topic[$key] 	= $get_topic['gask_topic'];
				$dission_topic_id[$key] = $get_topic['group_task_id'];	
			}
			$topic_number = 0;
			$topic_number = count($dission_topic_id);
			//-----------------------------------------------
			echo json_encode(array("Arrtotal"=>$Arrtotal,"Arrtotal_process"=>$Arrtotal_process,"achieve_type"=>$achieve_type,"title_name"=>$title_name,"title_score"=>$title_score,"title_next"=>$title_next,"get_score"=>$get_score,"sex"=>$sex,"dission_topic"=>$dission_topic,"dission_topic_id"=>$dission_topic_id,"topic_number"=>$topic_number));
		...
		case 5: //鷹架發文
			...
			";
			$achieve_data =db_result($conn_type='pdo',$conn_mssr,$sql,array(),$arry_conn_mssr);
			if(isset($achieve_data[0]['cno'])){$cno =(int)$achieve_data[0]['cno'];}
