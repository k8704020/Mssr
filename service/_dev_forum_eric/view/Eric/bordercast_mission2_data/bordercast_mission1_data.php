    ...
    //---------------------------------------------------
		...
		$sql="
			...
			FROM `mssr_forum`.`dev_complete_mission_log` as a
				...
				LEFT JOIN  `user`.`school` AS sch1 ON ( sch1.`school_code` = sem1.`school_code` )
			WHERE 
				a.`group_task_id` =5 and a.`deliver_uid` in ($output3)
				and sem1.`semester_code` like '%2015_2%' 
				and p.`status` in ('i_s','i_t') 
			GROUP BY a.`master_ctask_id`
		";
		...
		$sql="
			...
			FROM `mssr_forum`.`dev_complete_mission_log` as a
				...
				LEFT JOIN  `user`.`school` AS sch1 ON ( sch1.`school_code` = sem1.`school_code` )
			WHERE 
				a.`group_task_id` =5 and a.`deliver_uid` in ($output4)
				and sem1.`semester_code` like '%2015_2%' 
				and p.`status` in ('i_s','i_t') 
			GROUP BY a.`master_ctask_id`
		";
		...
		$sql="
			...
			FROM `mssr_forum`.`dev_complete_mission_log` as a
				...
				LEFT JOIN  `user`.`school` AS sch1 ON ( sch1.`school_code` = sem1.`school_code` )
			WHERE 
				a.`group_task_id` =5 and a.`deliver_uid` in ($output5)
				and sem1.`semester_code` like '%2015_2%' 
				and p.`status` in ('i_s','i_t') 
			GROUP BY a.`master_ctask_id`
		";
		$get_data6 = db_result($conn_type='pdo',$conn_mssr,$sql,array(),$arry_conn_mssr);
		foreach($get_data6 as $key => $get_data6){
			
			$mission_state6[$key]= $get_data6['mission_state'];
			$number6++;
			if($mission_state6[$key]==1||$mission_state6[$key]==2){
				$accept6++;
			}
			if($mission_state6[$key]==1){
				$query6[$n6] 		 = $get_data6['accept_uid'];
				$n6++;
				$complete6++;
			}
			if($mission_state6[$key]==3){
				$reject6++;
			}
			if($mission_state6[$key]==5){
				$no_reply6++;
			}
		}
		$output6=implode(",",$query6);	
		//-------------------------------------------------
		//-------------------------------------------------
		//[第七層]
		$query7 = array();
		...
		$sql="
			...
			FROM `mssr_forum`.`dev_complete_mission_log` as a
				...
				LEFT JOIN  `user`.`school` AS sch1 ON ( sch1.`school_code` = sem1.`school_code` )
			WHERE 
				a.`group_task_id` =5 and a.`deliver_uid` in ($output6)
				and sem1.`semester_code` like '%2015_2%' 
				and p.`status` in ('i_s','i_t') 
			GROUP BY a.`master_ctask_id`
		";
		$get_data7 = db_result($conn_type='pdo',$conn_mssr,$sql,array(),$arry_conn_mssr);
		foreach($get_data7 as $key => $get_data7){
			
			$mission_state7[$key]= $get_data7['mission_state'];
			$number7++;
			if($mission_state7[$key]==1||$mission_state7[$key]==2){
				$accept7++;
			}
			if($mission_state7[$key]==1){
				$query7[$n7] 		 = $get_data7['accept_uid'];
				$n7++;
				$complete7++;
			}
			if($mission_state7[$key]==3){
				$reject7++;
			}
			if($mission_state7[$key]==5){
				$no_reply7++;
			}
		}
		$output7=implode(",",$query7);	
		//-------------------------------------------------
		$total = 1320;//傳遞人數
		echo "</table>";
		echo "----------------------------";
		...
		echo "</br>";
		echo "[第五層]使用者傳播人數：".$number5."人 (來自".$n4."人)";
		echo "</br>";
		echo "任務傳遞者：".$output4;
		echo "</br>";
		...
		echo "</br>";
		echo "[第六層]使用者傳播人數：".$number6."人 (來自".$n5."人)";
		echo "</br>";
		echo "任務傳遞者：".$output5;
		echo "</br>";
		echo "&nbsp;&nbsp;有看到任務：".($accept6+$reject6+$no_reply6)."人";
		echo "</br>";
		echo "&nbsp;&nbsp;接受者：".$accept6."人&nbsp;(完成者：".$complete6."人)";
		echo "</br>";
		echo "&nbsp;&nbsp;拒絕者：".$reject6."人";
		echo "</br>";
		echo "&nbsp;&nbsp;>任務接受率：".(round($accept6/($accept6+$reject6+$no_reply6),2)*100)."%";
		echo "</br>";
		echo "&nbsp;&nbsp;>任務完成率：".(round($complete6/($accept6+$reject6+$no_reply6),2)*100)."%";
		echo "</br>";
		echo "[第七層]使用者傳播人數：".$number7."人 (來自".$n6."人)";
		echo "</br>";
		echo "任務傳遞者：".$output6;
		echo "</br>";
		echo "&nbsp;&nbsp;有看到任務：".($accept7+$reject7+$no_reply7)."人";
		echo "</br>";
		echo "&nbsp;&nbsp;接受者：".$accept7."人&nbsp;(完成者：".$complete7."人)";
		echo "</br>";
		echo "&nbsp;&nbsp;拒絕者：".$reject7."人";
		echo "</br>";
		echo "&nbsp;&nbsp;>任務接受率：".(round($accept7/($accept7+$reject7+$no_reply7),2)*100)."%";
		echo "</br>";
		echo "&nbsp;&nbsp;>任務完成率：".(round($complete7/($accept7+$reject7+$no_reply7),2)*100)."%";
		echo "</br>";
		echo "----------------------------";
