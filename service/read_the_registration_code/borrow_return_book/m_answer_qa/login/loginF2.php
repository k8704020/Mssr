...
//-------------------------------------------------------
    ...
    //---------------------------------------------------
        ...
        //$_SESSION['_read_the_registration_code']['opinion_book']['login']='loginF2.php';
        if($_SESSION['_read_the_registration_code']['opinion_book']['login']==='loginF'){
            $jscript_back="
                <script>
    ...
    //---------------------------------------------------
        ...
        //-----------------------------------------------
            ...
            //card_number  借書證號碼
                ...
                $sql="
                    SELECT `member`.`uid`, `member`.`name`, `library_card`.`card_number`
                    FROM `library_card`
                        INNER JOIN `member` ON
                        `library_card`.`uid`=`member`.`uid`
                    WHERE 1=1
                        AND `member`.`uid` IN ($users)
                    ORDER BY `member`.`uid` ASC
                ";
                //echo $sql.'<br/>';
                $db_results=db_result($conn_type='pdo',$conn_user,$sql,array(),$arry_conn_user);
...
<div id="container">
    ...
    <div id="content">
        ...
        <table id="read_the_registration_code_datalist_tbl" border="0" align="center" cellpadding="0" cellspacing="0">
            ...
            <tr>
                <td height="426px" valign='top' align="center">
                    <!-- 資料列表 開始 -->
                    <div style="margin-bottom:50px;margin-top:50px;">
                        <h1 class='fc_blue0'><b>請選擇學生來登入回答問題系統</b></h1>
                    </div>
                    <?php foreach($db_results as $db_result):?>
                    <?php
                        $rs_card_number=trim($db_result['card_number']);
                        $rs_name       =trim($db_result['name']);
                    ?>
                        <form action='loginA.php' method='post' style="float:left;">
                            <input type="text" id="card_number" name="card_number"
                            value="<?php echo htmlspecialchars($rs_card_number);?>" size="20" maxlength="30" tabindex="1"
                            class="form_text" style="width:150px;display:none;">
                            <input type="submit" name="<?php echo htmlspecialchars($rs_name);?>" value="<?php echo htmlspecialchars($rs_name);?>"
                            style="margin:5px 5px;width:120px;height:35px;"
...
//-------------------------------------------------------
    ...
    var oimg_opinion_book=document.getElementById('img_opinion_book');
