...
//-------------------------------------------------------
	...
	require_once(str_repeat("../",6)."service/read_the_registration_v2/inc/tx_gift_sid/code.php");
    require_once(str_repeat("../",6)."inc/set_user_vote_book_page/code.php");
    ...
    //---------------------------------------------------
		...
		$vote_page        =(isset($_POST['vote_page']))?(int)$_POST['vote_page']:0;	
	...
	//-------------------------------------------
		if($vote_page == 0 )
		{
			$array["echo"] ="填寫資料有問題喔，請再填一次";
			die(json_encode($array,1));
		}
		
		set_user_vote_book_page($conn_mssr,$user_id,$book_sid,$vote_page,$arry_conn_mssr);
		...
		$sql = "SELECT `book_sid`,`vote_state`,book_page 
