...
//-------------------------------------------------------
	...
	require_once(str_repeat("../",6)."inc/get_book_info/code.php");

	require_once(str_repeat("../",5)."bookstore_v2/inc/set_score_exp/code.php");
    ...
    //---------------------------------------------------
		...
		{
			$array["echo"] ="資料有點怪怪的喔！　請重新輸入<BR>CODE : 001".$borrow_sid."?".$book_sid."?".strlen($book_sid);
			die(json_encode($array,1));
		}
		
		$quest_list = json_decode($_POST['quest_list'],1);
		$re_answer = json_decode($_POST['re_answer'],1);
		$opinion_answer = array();
		
		
		foreach ($quest_list as $key => $vul)
		{
			$tmp = array();
			if($quest_list[$key]['type'] ==  "radio")
			{
				
				 $quest_list[$key]['quest']."：".$quest_list[$key]['answer'][$re_answer[$key]]."<BR>";
				$tmp['topic_id'] = $quest_list[$key]['topic_id'];
				$tmp['opinion_answer'] = array((string)$re_answer[$key]);
			}
			else if($quest_list[$key]['type'] ==  "number")
			{
				$tmp['topic_id'] = $quest_list[$key]['topic_id'];
				$tmp['opinion_answer'] = array((string)$re_answer[$key]);
			}
			else if($quest_list[$key]['type'] ==  "text")
			{
				$tmp['topic_id'] = $quest_list[$key]['topic_id'];
				$tmp['opinion_answer'] = array((string)$re_answer[$key]);
			}
			array_push($opinion_answer,$tmp);
		}
		//這是輸出
