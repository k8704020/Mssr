...
//-------------------------------------------------------
    ...
    //---------------------------------------------------
        ...
        $opinion_book_name='';
        if((isset($_SESSION['_read_the_registration_code']['_login']['opinion_book_name']))&&($_SESSION['_read_the_registration_code']['_login']['opinion_book_name']!=='')){
            $opinion_book_name=trim($_SESSION['_read_the_registration_code']['_login']['opinion_book_name']);
        }
            ...
            <table id="tbl_opinion_success" cellpadding="0" cellspacing="0" border="0" width="100%" class="table_style2" style="position:relative;top:40px;"/>
                <tr align="center" valign="middle">
                    <td width="170px" height="100px">
                        <span class='fc_red0 wp010-08 fsize_24'><?php if(isset($opinion_book_name)&&$opinion_book_name!=='')echo "【".htmlspecialchars($opinion_book_name)."】";?>登記成功！</span>
                    </td>
                </tr>
            </table>
        <?php endif;?>
            ...
            //成功處理
                ...
                switch(_book_code_type){
                    ...
                    case 'class':
                        if(_status!=='true'){
                        //班級無相關書籍
                            alert('書本不存在, 若書本有貼圖書館條碼, 請刷該條碼來借閱!');
                            location.reload();
                            if(first_borrow==='yes'){
                            //第一次借書
                                //載入書號輸入表單
...
<Body>
    ...
    <!-- 資料列表 開始 -->
        ...
        <!-- 班級選書  開始 -->
        <table id="tbl_class_book" width="100%" border="0" cellpadding="0" cellspacing="0" align="center" style="position:relative;display:none;"/>
            <tr>
                <!-- 在此設定寬高 -->
                <td width="100%" height="250px" align="center" valign="top">
                    <!-- 內容 -->
                    <table border="0" width="100%" cellpadding="5" cellspacing="0" style="" class="table_style2">
                        <tr>
                            <td id="td_class_book" height="470px" align="left" valign="top"></td>
                        </tr>
                    </table>
                </td>
            </tr>
...
//-------------------------------------------------------
                            ...
                            if(_book_numrow>1){
                                ...
                                for(var key in arry_book_names){
                                    ...
                                    if(book_name!==''){
                                        var _html='';
                                           _html+='<table align="left" cellpadding="0" cellspacing="0" border="0" style="width:160px;">';
                                           _html+='<tr><td align="center"><img book_name="'+book_name+'" book_code="'+_book_code+'" onclick="muilt_class_process(this);" src="../../img/book.png" width="50" height="50" border="0" alt="book" onmouseover="this.style.cursor='+"'"+'pointer'+"'"+';"></td></tr>';
                                           _html+='<tr><td class=" fsize_17" align="center">';
                                           _html+=book_name;
                                           _html+='</td></tr>';
                                           _html+='</table>';
                                        $('#td_class_book').append(_html);
                                    }
                                }
                            }else{
                                arry_book_names=_book_name.split(",");
                                _book_name=arry_book_names[0];
                                _sel_book_no(1,'single',_book_name);
                            }
                        }
                    break;
                }
    ...
    function add_unverified_book(){
    }

    function muilt_class_process(obj){
    //复选ISBN的书
        var book_code=obj.getAttribute('book_code');
        var book_name=obj.getAttribute('book_name');
        var url ='';
        var page=str_repeat('../',0)+'add/book_class_opinion_process/addA.php';
        var arg ={
            'psize':psize,
            'pinx':pinx,
            'book_code':book_code,
    ...
    function _sel_book_no(book_no,sel_type,book_name){
        var _book_code=trim(obook_code.value);
        var _book_no=parseInt(book_no);
        var _book_name=trim(book_name);
