    ...
    //---------------------------------------------------
        ...
        //側欄-群
            ...
            //訊息1
                ...
                //否，顯示5個小組成員(書友以及書友的書友優先顯示)，及誘導加入小組的字眼
                    $arry_msg1['main']=array();
                    foreach($arrys_group as $rs_user_id){
                        if(in_array($rs_user_id,$arry_good_friend_info)||in_array($rs_user_id,$arry_good_friend_friend_info)){
                            if(count($arry_msg1['main'])<5){
                    ...
                    if(!empty($db_results)){
                        foreach($db_results as $inx=>$db_result){
                            ...
                            if(isset($arry_msg1['main'][$inx])){
                                ...
                                //書名簡化，誰也參加這個聊書小組--也看過的書籍
                                if(mb_strlen($rs_book_name)>8){
                                    $rs_book_name_substr=mb_substr($rs_book_name,0,8)."..";
                                }else{
                                    $rs_book_name_substr = $rs_book_name;
                                }
                                //$rs_book_name_substr=mb_substr($rs_book_name,0,8)."..";
                                $arry_msg1['submain'][]="<a href='article.php?get_from=1&book_sid={$rs_book_sid}' style='text-decoration:none;' data-toggle='tooltip' data-placement='left' title='{$rs_book_name}'>".$rs_book_name_substr."</a>";
                            }
                        }
                    }
                }
                ...
                if(!empty($db_results)){
                    ...
                    if(!empty($arry_msg2['main'])){
                        foreach($arry_msg2['main'] as $inx=>$val){
                            ...
                            //$rs_book_name_substr=mb_substr($rs_book_name,0,8)."..";
                            $arry_msg2['main'][$inx]="<a href='article.php?get_from=1&book_sid={$val}' style='text-decoration:none;' data-toggle='tooltip' data-placement='left' title='{$rs_book_name}'>".$rs_book_name_substr."</a>";;
                        }
                    }
                }else{
            ...
            //訊息3
                ...
                $sql="
                    ...
                    WHERE 1=1
                        ...
                        AND `mssr_forum`.`mssr_forum_group_user_rev`.`user_state`=1
                    ORDER BY  `mssr_forum_group_user_rev`.`keyin_mdate` DESC 
                ";
                $db_results=$this->db_result($conn_type='pdo',$conn_mssr,$sql,$arry_limit=array(),$arry_conn_mssr);
                if(!empty($db_results)){
                    foreach($db_results as $db_result){
                ...
                if($merge_good_friend_info!==''){
                    ...
                    $sql="
                        ...
                        WHERE 1=1
                            ...
                            AND `mssr_forum`.`mssr_forum_group`.`group_id` IN
                            (
                                SELECT `mssr_forum`.`mssr_forum_group_user_rev`.`group_id`
                                FROM `mssr_forum`.`mssr_forum_group_user_rev`
                                WHERE `mssr_forum`.`mssr_forum_group_user_rev`.`user_state` = 1
                                GROUP BY `mssr_forum`.`mssr_forum_group_user_rev`.`group_id`
                            )
                    ";
                    if(isset($filter_1)&&$filter_1!==''){
                        $sql.="
                            AND `mssr_forum`.`mssr_forum_group`.`group_id` NOT IN ($filter_1)
                        ";
                    }
                    if(isset($filter_2)&&$filter_2!==''){
                        $sql.="
                            ...
                            (
                                SELECT `mssr_forum`.`mssr_forum_group_user_rev`.`group_id`
                                FROM `mssr_forum`.`mssr_forum_group_user_rev`
                                WHERE `mssr_forum`.`mssr_forum_group_user_rev`.`user_id` IN ($filter_2)
                                GROUP BY `mssr_forum`.`mssr_forum_group_user_rev`.`group_id`
                            )
                        ";
                    }
                ...
                if(!empty($arry_group_b) || !empty($arry_group_a)){
                    ...
                    if(!empty($arry_group)){
                        ...
                        foreach($arry_group as $inx=>$rs_group_id){
                            ...
                            //小組名簡化
                            if(mb_strlen($rs_group_name)>8){
                                $rs_group_name_substr=mb_substr($rs_group_name,0,8)."..";
                            }else{
                                $rs_group_name_substr = $rs_group_name;
                            }
                            //$rs_group_name_substr=mb_substr($rs_group_name,0,8)."..";
                            $arry_msg3['main'][]="<a href='article.php?get_from=2&group_id={$rs_group_id}' style='text-decoration:none;' data-toggle='tooltip' data-placement='left' title='{$rs_group_name}'>".$rs_group_name_substr."</a>";
                        }
                    }
                }else{
                ...
                //輔助, 並個別顯示那些書友參與過該聊書小組(取一筆)
                //輔助, 再個別顯示(幾位書友數-1)參與過該聊書小組
        ...
        //側欄-書
            ...
            //---- 例外, 若比對無任何書友借閱過此書籍, 則撈出前五人借閱過此書籍即可 ----//
                    ...
                    if(!empty($arry_has_borrow_good_friend_info)){
                        ...
                        if(empty($arry_good_friend_book_borrow_info_tmp)){
                            ...
                            foreach($arry_good_friend_book_borrow_info as $inx=>$rs_book_sid){
                                ...
                                //$rs_book_name_substr=mb_substr($rs_book_name,0,8)."..";
                                $arry_msg2['main'][]="<a href='article.php?get_from=1&book_sid={$rs_book_sid}' style='text-decoration:none;' data-toggle='tooltip' data-placement='left' title='{$rs_book_name}'>".$rs_book_name_substr."</a>";
                                $borrow_cno=0;

                                //輔助, 並個別顯示哪些書友以及書友的書友借閱過該本書(取一筆)
                                //輔助, 再個別顯示(幾位書友以及書友的書友數-1)借閱過該本書
                            ...
                            if($inx<5){
                                ...
                                if(!empty($db_results)){
                                    ...
                                    if(!in_array($rs_book_sid,$arry_user_book_borrow_info['book_sid'])){
                                        ...
                                        //$rs_book_name_substr=mb_substr($rs_book_name,0,8)."..";
                                        $arry_msg3['submain'][]="<a href='article.php?get_from=1&book_sid={$rs_book_sid}' style='text-decoration:none;' data-toggle='tooltip' data-placement='left' title='{$rs_book_name}'>".$rs_book_name_substr."</a>";
                                    }
                                }
                            }
                        }
                ...
                if(!empty($arry_intersect_book_borrow_info)){
                    ...
                    foreach(array_values($arry_intersect_book_borrow_info) as $rs_book_sid){
                        if($cno<5){
                            ...
                            //輔助, 並個別顯示有另外哪些(登入者)的書友半年內也讀這本書(取一筆書友)
                            //輔助, 再個別顯示(幾位書友-1)借閱過該本書
                ...
                if(!empty($arry_intersect_group_info)){
                    ...
                    foreach($arry_intersect_group_info as $rs_group_id){
                        if($cno<5){
                            ...
                            //$rs_group_name_substr=mb_substr($rs_group_name,0,8)."..";
                            $arry_msg3['main'][]="<a href='article.php?get_from=2&group_id={$rs_group_id}' style='text-decoration:none;' data-toggle='tooltip' data-placement='left' title='{$rs_group_name}'>".trim($rs_group_name_substr)."</a>";

                            //輔助, 並個別顯示有另外那些(登入者)的書友也參與該聊書小組(取一筆書友)
                            shuffle($arry_user_good_friend_info);
                        ...
                        if($rs_user_id!==$user_id){
                            ...
                            if(!empty($arry_book_borrow_info['cno'])&&!empty($arry_my_book_borrow_info['cno'])){
                                ...
                                foreach($arry_book_borrow_info['cno'] as $rs_book_sid=>$rs_cno){
                                    ...
                                    //$rs_book_name_substr=mb_substr($rs_book_name,0,8)."..";
                                    $arry_msg1['submain'][]="<br><a href='article.php?get_from=1&book_sid={$rs_book_sid}' style='text-decoration:none;' data-toggle='tooltip' data-placement='left' title='{$rs_book_name}'>".$rs_book_name_substr."</a>";
                                    break;
                                }
                            }
                        }
