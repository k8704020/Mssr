<?php
//-------------------------------------------------------
//函式: get_blacklist_reply_school()
//用途: 提取學校回覆黑名單資訊
//-------------------------------------------------------

    function get_blacklist_reply_school($school_code,$reply_id,$arry_conn){
    //---------------------------------------------------
    //函式: get_blacklist_reply_school()
    //用途: 提取學校回覆黑名單資訊
    //---------------------------------------------------
    ...
    //---------------------------------------------------
        ...
        //-----------------------------------------------
            ...
            if(!isset($school_code)||(trim($school_code)==='')){
                $err='GET_BLACKLIST_REPLY_SCHOOL:NO SCHOOL_CODE';
                die($err);
            }else{
                $school_code=trim($school_code);
            }

            if(!isset($reply_id)||(int)($reply_id)===0){
                $err='GET_BLACKLIST_REPLY_SCHOOL:NO REPLY_ID';
                die($err);
            }else{
            ...
            if(!isset($arry_conn)||(empty($arry_conn))){
                $err='GET_BLACKLIST_REPLY_SCHOOL:NO ARRY_CONN';
                die($err);
            }
        ...
        //-----------------------------------------------
            ...
            //-------------------------------------------
                ...
                try{
                    ...
                    $err ='GET_BLACKLIST_REPLY_SCHOOL:CONNECT FAIL';
                    die($err);
                }
        ...
        //-----------------------------------------------
            ...
            $db_results=array();
            $sql="
                SELECT `mssr_forum`.`mssr_forum_blacklist_reply_school`.`reply_id`
                FROM  `mssr_forum`.`mssr_forum_blacklist_reply_school`
                WHERE 1=1
                    AND `mssr_forum`.`mssr_forum_blacklist_reply_school`.`school_code`= '{$school_code  }'
                    AND `mssr_forum`.`mssr_forum_blacklist_reply_school`.`reply_id`   =  {$reply_id     }
                LIMIT 1
            ";
