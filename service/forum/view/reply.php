...
if (isset($sess_school_code) && in_array($sess_school_code, $isolated_school_array)) {
	...
	//---------------------------------------------------
		...
		////新增文章瀏覽次數log
		//add_forum_article_view_log($sess_user_id,$get_article_id,$arry_conn_mssr);
	...
	//---------------------------------------------------
		...
		//-----------------------------------------------
			...
			$sql="
				SELECT
					...
					IFNULL((
						...
						WHERE 1=1
							AND `user`.`student`.`uid` = `mssr_forum`.`mssr_forum_article`.`user_id`
			...
			if(empty($article_results)){
				...
				if(mb_strlen($rs_article_book_name)>20){
					$rs_article_book_name=trim(mb_substr($rs_article_book_name,0,20)."...");
				}
		...
		//-----------------------------------------------
			...
			$get_rank_and_point = get_rank_and_point($rs_article_user_id,'','','',$arry_conn_mssr);
			$rank = $get_rank_and_point['total_rank'];
			$appellation = get_appellation($rank,'','','',$arry_conn_mssr);
				...
				SELECT
					...
					IFNULL((
						...
						FROM `user`.`student`
							...
							`user`.`class`.`semester_code`=`user`.`semester`.`semester_code`
						WHERE 1=1
							AND `user`.`student`.`uid` = `mssr_forum`.`mssr_forum_reply`.`user_id`
							AND NOW() BETWEEN `user`.`semester`.`start` AND `user`.`semester`.`end`
						GROUP BY `user`.`semester`.`school_code`
						LIMIT 1
					),'') AS `school_code`,
		...
		//-----------------------------------------------
			...
			if(isset($sess_user_id)){
				...
				//點數為負數時提出警告
				$warning = null;
				$point_gap = -$user_point;

				if ($user_point <= -1 && $user_point >= -150) {
					$warning = "※大家目前對你的評價有點低，你可以多認真回覆一些其他人的文章來改善這個狀況。";
				}
				if ($user_point <= -151 && $user_point >= -300) {
					$warning = "※大家目前對你的評價很低，你可以多認真回覆一些其他人的文章來改善這個狀況。";
				}
				if ($user_point <= -301 && $user_point >= -500) {
					$warning = "※大家目前對你的評價非常低，如果評價再更低的話，你將無法進行回文<br>&nbsp;&nbsp;&nbsp;&nbsp;你需要更認真的回覆一些其他人的文章來改善這個狀況。";
				}
			}
			...
			if(isset($sess_user_id)){
				if($sess_user_id!==$rs_article_user_id){
					...
					if(empty($get_forum_friend)){
						...
						if((int)$get_forum_friend[0]['friend_state']===3){$btn_add_friend_show=true;$btn_add_friend_html=trim('書友確認中');}
					}
				}
			}
			...
			if(isset($sess_user_id)){
				...
				$sql="
					...
					WHERE 1=1
						AND `mssr_forum`.`mssr_forum_article_like_log`.`user_id`   ={$sess_user_id }
						AND `mssr_forum`.`mssr_forum_article_like_log`.`article_id`={$rs_article_id}
						AND `mssr_forum`.`mssr_forum_article_like_log`.`like_status` = 1
				";
				$like_article_results2=db_result($conn_type='pdo',$conn_mssr,$sql,array(),$arry_conn_mssr);
				$sql="
					SELECT `mssr_forum`.`mssr_forum_article_like_log`.`user_id`
					FROM  `mssr_forum`.`mssr_forum_article_like_log`
					WHERE 1=1
						...
						AND `mssr_forum`.`mssr_forum_article_like_log`.`like_status` = 1
				";
				$like_article_results3=db_result($conn_type='pdo',$conn_mssr,$sql,array(),$arry_conn_mssr);
				$like_article_cno    =count($like_article_results3);
				if(!empty($like_article_results) && !empty($like_article_results2)){
	...
	<div class="container">
		...
		<div class="row">
			...
			<div class="jumbotron jumbotron-xs hidden-sm hidden-md hidden-lg">
				...
				onclick="location.href='article.php?get_from=1&book_sid=<?php echo addslashes($get_book_sid);?>'"
				onmouseover='this.style.cursor="pointer";'><?php echo htmlspecialchars($book_name);?></span>
			...
			<div class="book_lefe_side col-xs-12 col-sm-10 col-md-10 col-lg-10">
				...
				style="border:0px dashed transparent;">
					<li role="presentation" class="active hidden"><a href="#view_article" id="home-tab" role="tab" data-toggle="tab" aria-controls="home" aria-expanded="true">
						討論串
					</a></li>
					<?php if($auth_edit_book_info):?>
				...
				<div id="myTabContent" class="tab-content">
					...
					<div role="tabpanel" class="tab-pane fade in active" id="view_article" aria-labelledBy="home-tab">
						...
						<ol class="breadcrumb" style="font-size:11pt;">
							<li><a href="index.php">明日聊書</a></li>
							<li><a href="article.php?get_from=1&book_sid=<?php echo addslashes($get_book_sid);?>">
								<?php echo mb_substr(htmlspecialchars($book_name),0,10);?>
								<?php if(mb_strlen($book_name)>10)echo '...';?>
							</a></li>
							<li class="active">
								<?php echo mb_substr(htmlspecialchars($rs_article_title),0,10);?>
								<?php if(mb_strlen($rs_article_title)>10)echo '...';?>
							</li>
						</ol>
						...
						<div class="row">
							<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
								<div class="media">
									...
									<h4 class="media-heading" style="background-color: #cccccc;">
										<?php 
											$mark = "<span style='font-weight: normal;'>" . $appellation_mark . "</span>";
											echo htmlspecialchars($rs_article_user_name);
										 ?>
	...
	echo "（頭銜：<span style='color:$font_color;background-color:$background_color;'>" . $mark . $appellation_name . $mark . "</span>）";
										...
										<?php if(isset($sess_user_id)):?>
											...
											<?php if($my_has_borrow_flag):?>
												...
												data-toggle="modal" data-target=".modal_request_article">邀請書友一同聊書</button>
											<?php endif;?>
											...
											<?php if((in_array('i_t',$arry_user_status)&&$rs_article_school_code!==$sess_school_code)||in_array('i_a',$arry_user_status)):?>
												...
												article_id=<?php echo $rs_article_id;?>>隱藏文章</button>
											<?php endif?>
											...
											<?php if($my_has_borrow_flag):?>
												...
												user_id=<?php echo $sess_user_id;?>
												article_id=<?php echo $rs_article_id;?>
												user_name=<?php echo $rs_article_user_name;?>
											...
											<div class="btn-group pull-right hidden-sm hidden-md hidden-lg">
												...
												<ul class="dropdown-menu" role="menu">
													<?php if($my_has_borrow_flag):?>
														...
														data-toggle="modal" data-target=".modal_request_article">邀請書友一同聊書</a></li>
													<?php endif;?>
													...
													<?php if((in_array('i_t',$arry_user_status)&&$rs_article_school_code!==$sess_school_code)||in_array('i_a',$arry_user_status)):?>
														...
														article_id=<?php echo $rs_article_id;?>>隱藏文章</a></li>
													<?php endif?>
										...
										<div style="position:relative;">
											<?php echo htmlspecialchars($rs_article_keyin_cdate);?>&nbsp;#1
											<a href="article.php?get_from=1&book_sid=<?php echo addslashes($rs_article_book_sid);?>" style='color:#428bca;'>
												<span class="hidden-xs" style="position:absolute;right:5px;color:#428bca;">
													<img width="30" height="30" src="<?php echo $rs_article_book_img;?>" border="0">
									...
									<div class="pull-right media-body visible-xs">
										<a href="article.php?get_from=1&book_sid=<?php echo addslashes($rs_article_book_sid);?>" style='color:#428bca;'>
						...
						<?php if(!empty($reply_results)){
							foreach($reply_results as $inx=>$reply_result):
								...
								if(mb_strlen($rs_reply_book_name)>20){
									$rs_reply_book_name=trim(mb_substr($rs_reply_book_name,0,20)."...");
								}
								...
								//取得各樓層的頭銜
								$get_rank_and_point = get_rank_and_point($rs_reply_user_id,'','','',$arry_conn_mssr);
								$rank = $get_rank_and_point['total_rank'];
								$appellation = get_appellation($rank,'','','',$arry_conn_mssr);
								$appellation_name = $appellation["appellation_name"];
								$appellation_mark = $appellation["appellation_mark"];
								...
								if(isset($sess_user_id)){
									...
									$sql="
										...
										WHERE 1=1
											AND `mssr_forum`.`mssr_forum_reply_like_log`.`user_id` ={$sess_user_id }
											AND `mssr_forum`.`mssr_forum_reply_like_log`.`reply_id`={$rs_reply_id  }
											AND `mssr_forum`.`mssr_forum_reply_like_log`.`like_status` = 1
									";
									$like_reply_results2=db_result($conn_type='pdo',$conn_mssr,$sql,array(),$arry_conn_mssr);
									$sql="
										SELECT `mssr_forum`.`mssr_forum_reply_like_log`.`user_id`
										FROM  `mssr_forum`.`mssr_forum_reply_like_log`
										WHERE 1=1
											...
											AND `mssr_forum`.`mssr_forum_reply_like_log`.`like_status` = 1
									";
									$like_reply_results3=db_result($conn_type='pdo',$conn_mssr,$sql,array(),$arry_conn_mssr);
									$like_reply_cno    =count($like_reply_results3);
									if(!empty($like_reply_results) && !empty($like_reply_results2)){
							...
							<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
								<div class="media">
									...
									<h4 class="media-heading">
										<?php 
											$mark = "<span style='font-weight: normal;'>" . $appellation_mark . "</span>";
											echo htmlspecialchars($rs_reply_user_name); ?> 
<?php if ($use_new_system) {
	echo "（頭銜：<span style='color:$font_color;background-color:$background_color;'>" . $mark . $appellation_name . $mark . "</span>）";
} ?>
										...
										<?php if(isset($sess_user_id)):?>
											...
											<?php if((in_array('i_t',$arry_user_status)&&$rs_reply_school_code!==$sess_school_code)||in_array('i_a',$arry_user_status)):?>
												...
												reply_id=<?php echo $rs_reply_id;?>>隱藏回覆</button>
											<?php endif?>
											...
											<?php if($my_has_borrow_flag):?>
												...
												user_id=<?php echo $sess_user_id;?>
												article_id=<?php echo $rs_article_id;?>
												user_name=<?php echo $rs_reply_user_name;?>
											...
											<div class="btn-group pull-right hidden-sm hidden-md hidden-lg">
												...
												<ul class="dropdown-menu" role="menu">
													...
													<?php if((in_array('i_t',$arry_user_status)&&$rs_reply_school_code!==$sess_school_code)||in_array('i_a',$arry_user_status)):?>
														...
														reply_id=<?php echo $rs_reply_id;?>>隱藏回覆</a></li>
													<?php endif?>
										...
										<div style="position:relative;">
											<?php echo htmlspecialchars($rs_reply_keyin_cdate);?>&nbsp;#<?php echo ($pinx*$psize)-$psize+2;?>
											<a href="article.php?get_from=1&book_sid=<?php echo addslashes($rs_reply_book_sid);?>" style='color:#428bca;'>
												<span class="hidden-xs" style="position:absolute;right:5px;color:#428bca;">
													<img width="30" height="30" src="<?php echo $rs_reply_book_img;?>" border="0">
						...
						<div id="reply_article_row" class="row">
							<div id="reply_article" class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
														...
														<div class="col-xs-12 col-sm-12 col-md-12 col-lg12">
															<form id="Form1" name="Form1" method="post" onsubmit="return false;">
																<div class="div_textarea form-group" style="margin-top:15px;">
																	<textarea class="form-control" id="reply_content[]" name="reply_content[]" rows="6" placeholder="" style="resize:none;"></textarea>
																</div>
								...
								<?php if(!$enough_point): ?>
									<div class="col-xs-12 col-sm-12 col-md-12 col-lg12">
										<font color="red">
											<br>※由於大家對你的評價過低，你現在無法進行回文，你可以請求老師解除這個懲罰。
										</font>
									</div>
								<?php endif;?>
							</div>
...
//-------------------------------------------------------
	...
	function clone_request_article_tag(){
		...
		$('.request_article_group').each(function(){
			...
			$request_article_group.find("A").click(function(){
				auto($(this)[0],parseInt($request_article_group.index()-2));
			});
		});
	...
	$(function(){
		...
		//參數設置
			...
			datatype   :"json",
			data       :{
				user_id     :encodeURI(trim(sess_user_id            )),
				article_id  :encodeURI(trim(get_article_id          )),
				method      :encodeURI(trim('add_forum_article_view')),
				send_url    :encodeURI(trim(send_url                ))
			},
	...
	<div class="container">
		...
		<div class="row">
			...
			<div class="jumbotron jumbotron-xs hidden-sm hidden-md hidden-lg" style="background-image:url('<?php echo $front_cover_group_1_img;?>');background-position:center top;background-size:100% auto;">
				...
				<span class="jumbotron-xs_name"
				onclick="location.href='article.php?get_from=2&group_id=<?php echo $get_group_id;?>'"
				onmouseover='this.style.cursor="pointer";'><?php echo htmlspecialchars($group_name);?></span>
				<!-- jumbotron-xs_name,end -->
			...
			<div class="group_lefe_side col-xs-12 col-sm-10 col-md-10 col-lg-10">
				...
				<div id="myTabContent" class="tab-content">
					...
					<div role="tabpanel" class="tab-pane fade in active" id="view_article" aria-labelledBy="home-tab">
						...
						<ol class="breadcrumb" style="font-size:11pt;">
							<li><a href="index.php">明日聊書</a></li>
							<li><a href="article.php?get_from=2&group_id=<?php echo $get_group_id;?>">
								<?php echo mb_substr(htmlspecialchars($group_name),0,10);?>
								<?php if(mb_strlen($group_name)>10)echo '...';?>
							</a></li>
							<li class="active">
								<?php echo mb_substr(htmlspecialchars($rs_article_title),0,10);?>
								<?php if(mb_strlen($rs_article_title)>10)echo '...';?>
