                ...
                if($wall_sql!==''){
                    ...
                    if(!empty($db_results)){
                        ...
                        foreach($db_results as $db_result){
                            ...
                            $tmp_arry_article_id[]=$rs_article_id;
                        ...
                        $tmp_arry_article_id=array_unique($tmp_arry_article_id);
                        $tmp_group_id       =implode(",",$tmp_arry_group_id);
                        $tmp_book_sid       =implode("','",$tmp_arry_book_sid);
                        $tmp_article_id     =implode(",",$tmp_arry_article_id);
                        ...
                        if(trim($tmp_group_id)!==''){
                            ...
                            if(isset($sess_school_code)&&trim($sess_school_code)!==''){
                                $sql="
                                    ...
                                    WHERE 1=1
                                        AND `mssr_forum`.`mssr_forum_blacklist_article_school`.`school_code`= '{$sess_school_code  }'
                                        AND `mssr_forum`.`mssr_forum_blacklist_article_school`.`article_id`   IN ($tmp_article_id)
                                ";
                                $db_results=db_result($conn_type='pdo',$conn_mssr,$sql,array(),$arry_conn_mssr);
                                if(!empty($db_results)){
                                    foreach($db_results as $db_result){
                                        ...
                                        $rs_article_id   =(int)$rs_article_id;
                                        foreach($wall_results as $inx=>$wall_result){
                                            if($rs_article_id===(int)$wall_result['article_id']){
                                                unset($wall_results[$inx]);
                                            }
                                        }
                                    }
                        ...
                        if(trim($tmp_book_sid)!==''){
                            ...
                            foreach($wall_results as $inx=>$wall_result){
                                extract($wall_result, EXTR_PREFIX_ALL, "rs");
                                ...
                                $wall_results[$inx]['like_article_cno']=$like_article_cno;

                                //觀看數
                                $sql="
                                    SELECT `mssr_forum`.`mssr_forum_article_view_log`.`user_id`
                                    FROM `mssr_forum`.`mssr_forum_article_view_log`
                                    WHERE 1=1
                                        AND `mssr_forum`.`mssr_forum_article_view_log`.`article_id`={$rs_article_id}
                                ";
                                $article_view_results=db_result($conn_type='pdo',$conn_mssr,$sql,array(),$arry_conn_mssr);
                                $article_view_cno    =count($article_view_results);
                                $wall_results[$inx]['article_view_cno']=$article_view_cno;
// echo "h";
                            }
                            //echo "<Pre>";print_r($wall_results);echo "</Pre>";
                        }
                    } 
                }
            }
        ...
        //-----------------------------------------------
            ...
            // $arry_guide_books=[];
            // $sql="
            //     SELECT `mssr`.`mssr_book_borrow_semester`.`book_sid`
            //     FROM `mssr`.`mssr_book_borrow_semester`
            //     WHERE 1=1
            //         AND `mssr`.`mssr_book_borrow_semester`.`user_id`={$sess_user_id}
            //     GROUP BY `mssr`.`mssr_book_borrow_semester`.`user_id`,
            ...
            //         $arry_guide_books[$inx]['book_sid']=trim($book_result['book_sid']);
            //     }
            //     foreach($arry_guide_books as $inx=>$arry_guide_book){
            //         $rs_book_sid=trim($arry_guide_book['book_sid']);
            ...
            //         }
            //         $arry_guide_books[$inx]['book_img']=$rs_book_img;
            //         $get_book_info=get_book_info($conn_mssr,$rs_book_sid,$array_filter=array('book_name','book_note'),$arry_conn_mssr);
            //         if(!empty($get_book_info)){
            //             $arry_guide_books[$inx]['book_name']=trim($get_book_info[0]['book_name']);
            //             $arry_guide_books[$inx]['book_note']=trim($get_book_info[0]['book_note']);
            //         }else{unset($arry_guide_books[$inx]);}
            //     }
            //     foreach($arry_guide_books as $inx=>$arry_guide_book){
            //         $rs_book_note=trim($arry_guide_book['book_note']);
            //         $rs_book_name=trim($arry_guide_book['book_name']);
            //         if(mb_strlen($rs_book_name)>3){
            //             $arry_guide_books[$inx]['book_name']=mb_substr($rs_book_name,0,10)."..";
            //         }
            //         if($rs_book_note==='' && count($arry_guide_books)>1)unset($arry_guide_books[$inx]);
            //     }
            //     shuffle($arry_guide_books);
            //     //if($arry_guide_books[0]['book_note']===''){
            //         $arry_guide_books[0]['book_note']="讀過了這本書籍後，可以與大家一起討論書本的內容、表達讀完書後的感受，或是提出關於這本書的疑問與發現。";
            //     //}
            // }
            //echo "<Pre>";print_r($arry_guide_books);echo "</Pre>";
            ...
            //         `mssr_forum`.`mssr_forum_group`.`create_by`=`user`.`personnel`.`uid`
            //     WHERE 1=1
            ...
            //     if($sess_school_code!==''){
            //         //$sql.="
            //         //    AND `user`.`personnel`.`school_code`='{$sess_school_code}'
            //         //";
            //     }
            // }
            // $sql.="
            ...
            //     shuffle($group_results);
            //     foreach($group_results as $inx=>$group_result){
            //         $rs_group_id    =(int)($group_result['group_id']);
            //         $rs_group_name  =trim($group_result['group_name']);
            //         $rs_group_img   ='../img/default/group.jpg';
            //         if(@getimagesize("http://".$arry_ftp1_info['host']."/mssr/info/forum/group/{$rs_group_id}/group_sticker/1.jpg")){
            ...
            //         }
            //         $arry_guide_groups[$inx]['group_name']   =trim($rs_group_name);
            //         $arry_guide_groups[$inx]['group_content']=trim($group_result['group_content']);
            //         $arry_guide_groups[$inx]['group_id']     =(int)($group_result['group_id']);
            //         $arry_guide_groups[$inx]['group_img']    =trim($rs_group_img);
            //         if($rs_group_img==='../img/default/group.jpg' && count($arry_guide_groups)>1)unset($arry_guide_groups[$inx]);
            //     }
            //     shuffle($arry_guide_groups);
            //     //if($arry_guide_groups[0]['group_content']===''){
            //         $arry_guide_groups[0]['group_content']="加入小組，與大家一起討論共同的話題、書籍，以及參與各式各樣舉辦的小組活動。";
            //     //}
            // }
            //echo "<Pre>";print_r($arry_guide_groups);echo "</Pre>";
    ...
    <div class="container">
        ...
        <div class="row">
            ...
            <div class="wall col-xs-12 col-sm-10 col-md-10 col-lg-10">
                ...
                <!-- 引導資訊,start -->
                <div class="row rec_friend hidden" style="position:relative;margin-top:20px;">
                    ...
                    <div class="col-xs-4 col-sm-4 col-md-4 col-lg-4">
                        <?php if(isset($sess_school_code)):?>
                            <div class="thumbnail index_guide_thumbnail">
                                <?php
                                    $rs_user_id  =(int)$rec_friend_results[0]['uid'];
                                    $rs_user_sex =(int)$rec_friend_results[0]['sex'];
                                    $rs_user_name=trim($rec_friend_results[0]['name']);
                                    ...
                                    if(isset($file_server_enable)&&($file_server_enable)){
                                        if(@getimagesize("http://".$arry_ftp1_info['host']."/mssr/info/user/{$user_id}/forum/user_sticker/1.jpg")){
                                            $rs_user_img="http://".$arry_ftp1_info['host']."/mssr/info/user/{$user_id}/forum/user_sticker/1.jpg";
                                        }
                                    }else{
                                        if(file_exists("../../../info/user/{$user_id}/forum/user_sticker/1.jpg")){
                                            $rs_user_img="../../../info/user/{$user_id}/forum/user_sticker/1.jpg";
                                        }
                                    }
                                ?>
                                <a href="user.php?user_id=<?php echo $rs_user_id;?>">
                                    <img class='img-responsive' alt="推薦書友" src="<?php echo $rs_user_img;?>" style="height:150px;">
                                </a>
                                <div class="caption">
                                    <a href="user.php?user_id=<?php echo $rs_user_id;?>">
                                        <p class="text-center" style="font-size:15pt;color:#5cb85c;">
                                            <strong>你認識【<?php echo htmlspecialchars($rs_user_name);?>】嗎？</strong>
                                        </p>
                                        <p class="text-left" style="font-size:11pt;margin-left:10px;">
                                            尋找更多書友，你可以與他們討論讀過的書籍、加入他們的小組，也可以得知他們最新的動態消息。
                                        </p>
                                    </a>
                                </div>
                                <div class="input-group">
                                    <input type="text" name="index_search_value" class="form-control input-sm" placeholder="尋找更多書友" />
                                    <span class="input-group-btn">
                                        <button class="btn btn-success btn-sm" type="button" onclick="index_guide_search(1);">
                    ...
                    <div class="col-xs-4 col-sm-4 col-md-4 col-lg-4">
                        <div class="thumbnail index_guide_thumbnail">
                            <a href="article.php?get_from=1&book_sid=<?php echo addslashes($arry_guide_books[0]['book_sid']);?>">
                                <img class='img-responsive' alt="推薦書籍" src="<?php echo $arry_guide_books[0]['book_img'];?>" style="height:150px;">
                            </a>
                            <div class="caption">
                                <a href="article.php?get_from=1&book_sid=<?php echo addslashes($arry_guide_books[0]['book_sid']);?>">
                                    <p class="text-center" style="font-size:15pt;color:#5bc0de;">
                                        <strong>【<?php echo htmlspecialchars($arry_guide_books[0]['book_name']);?>】</strong>
                                    </p>
                                    <p class="text-left" style="font-size:11pt;margin-left:10px;">
                                        <?php echo htmlspecialchars($arry_guide_books[0]['book_note']);?>
                                    </p>
                                </a>
                            </div>
                            <div class="input-group">
                                <input type="text" name="index_search_value" class="form-control input-sm" placeholder="尋找更多書籍" />
                                <span class="input-group-btn">
                                    <button class="btn btn-info btn-sm" type="button" onclick="index_guide_search(2);">
                                        <i class="glyphicon glyphicon-search"></i>
                                    </button>
                                </span>
                            </div>
                    ...
                    <div class="col-xs-4 col-sm-4 col-md-4 col-lg-4">
                        <div class="thumbnail index_guide_thumbnail">
                            <a href="article.php?get_from=2&group_id=<?php echo addslashes($arry_guide_groups[0]['group_id']);?>">
                                <img class='img-responsive' alt="推薦小組" src="<?php echo $arry_guide_groups[0]['group_img'];?>" style="height:150px;">
                            </a>
                            <div class="caption">
                                <a href="article.php?get_from=2&group_id=<?php echo addslashes($arry_guide_groups[0]['group_id']);?>">
                                    <p class="text-center" style="font-size:15pt;color:#f0ad4e;">
                                        <strong>【<?php echo htmlspecialchars($arry_guide_groups[0]['group_name']);?>】</strong>
                                    </p>
                                    <p class="text-left" style="font-size:11pt;margin-left:10px;">
                                        <?php echo htmlspecialchars($arry_guide_groups[0]['group_content']);?>
                                    </p>
                                </a>
                            </div>
                            <div class="input-group">
                                <input type="text" name="index_search_value" class="form-control input-sm" placeholder="尋找更多小組" />
                                <span class="input-group-btn">
                                    <button class="btn btn-warning btn-sm" type="button" onclick="index_guide_search(3);">
                                        <i class="glyphicon glyphicon-search"></i>
                                    </button>
                                </span>
                            </div>
                ...
                <!-- 推薦書友,start -->
                <?php if(1===2 && isset($rec_friend_results)&&!empty($rec_friend_results)):?>
                    ...
                    <?php
                        ...
                        foreach($rec_friend_results as $rec_friend_result):
                            if($cno<4){
                                ...
                                if($rs_user_sex===2)$rs_user_img='../img/default/user_girl.png';
                    ?>
                        <div class="col-xs-6 col-sm-3 col-md-3 col-lg-3 rec_friend_content">
                            <div class="thumbnail">
                ...
                <?php if(isset($wall_results)&&!empty($wall_results)):?>
                    ...
                    foreach($wall_results as $rs_time=>$wall_result):
                        ...
                        $rs_like_article_cno=(int)$wall_result[trim('like_article_cno')];
                        $rs_article_view_cno=(int)$wall_result[trim('article_view_cno')];
                        ...
                        if($rs_group_id===0){
                            ...
                            if(@getimagesize("http://".$arry_ftp1_info['host']."/mssr/info/forum/group/{$rs_group_id}/group_sticker/1.jpg")){
                                $rs_img_1="http://".$arry_ftp1_info['host']."/mssr/info/forum/group/{$rs_group_id}/group_sticker/1.jpg";
                            }
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                        <div class="media" style="">
                            ...
                            <div class="media-body">
                                <h4 class="media-heading">歡迎蒞臨明日聊書！ 多加一些書友，你就可以看到更多動態消息。</h4>
                            </div>
                        </div>
                    </div>
                <?php endif;?>
            ...
            <div class="right_side col-xs-12 col-sm-2 col-md-2 col-lg-2">
                <div class="row hidden-xs" style="border:0px solid #dddddd;border-radius:5px;margin-top:0px;">
                    ...
                    <div class="index_left_side col-xs-12 col-sm-12 col-md-12 col-lg-12 hidden">
                        <a href="user.php?user_id=<?php //echo $sess_user_id;?>&tab=5">
                            <em class="glyphicon glyphicon-user"></em> 我的書友 <span class='hidden'>Friends</span>
                        </a>
                    </div>
                    <div class="index_left_side col-xs-12 col-sm-12 col-md-12 col-lg-12 hidden">
                ...
                <table class="table" style="margin-top:25px;">
                    ...
                    <tbody>
                        <?php if(!empty($hot_book_results)){
                            ...
                            foreach($hot_book_results as $rs_book_sid=>$hot_book_cno){
                                ...
                                if($cno<5){
                                    ...
                                    $all_book_name = $rs_book_name;     //顯示完整書名
                        ...
                        ?>
                        <tr><td><a href="article.php?get_from=1&book_sid=<?php echo addslashes($rs_book_sid);?>" style="text-decoration:none;" data-toggle="tooltip" data-placement="left" title="<?php echo $all_book_name;?>"><!-- 顯示完整書名 -->
...
<?php if ($use_new_system) { ?>
	<div id="new_system_message" class="modal fade"  tabindex="-1" role="dialog" aria-hidden="true">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="text-center">
						<img src="../img/new_system.png" width="100%">
					</div>
					<div class="text-center">
...
//-------------------------------------------------------
    ...
    //FUNCTION
    function index_guide_search(search_type){
        var search_type=parseInt(search_type);
        var oindex_search_values=document.getElementsByName('index_search_value');
        var url='query.php?search_value='+encodeURI(oindex_search_values[search_type-1].value)+'&search_type='+encodeURI(search_type);
        location.href=url;
    }
    ...
    //刷新推薦的書友
        ...
        for(key1 in json_rec_friend_results){
            ...
            html+='<div class="col-xs-6 col-sm-3 col-md-3 col-lg-3 rec_friend_content">';
