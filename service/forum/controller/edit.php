				...
				//---------------------------------------
					...
					$sql = "
						SELECT 
							...
							DATE(`mssr_forum`.`mssr_forum_user_request`.`keyin_cdate`) AS `keyin_cdate`
						FROM `mssr_forum`.`mssr_forum_user_request`
						WHERE 1=1
							AND `mssr_forum`.`mssr_forum_user_request`.`request_id` = $request_id;
					";
					...
					foreach ($sth as $row) {
						...
						$request_from = (int)$row["request_from"];
						$request_to = (int)$row["request_to"];
						$keyin_cdate = $row["keyin_cdate"];
					}
					...
					$sql = "
						...
						FROM `user`.`member`
						WHERE 1=1
							AND `user`.`member`.`uid` = $request_from;
					";

					$sth=$conn_mssr->prepare($sql);
					$sth->execute();
					foreach ($sth as $row) {
						$request_from_user_name = $row["name"];
					}

					//發文所獲得的積分
					$get_request_from_rank = 5;
					$get_request_to_rank = 20;

					//回覆推薦書籍獲得的親密度
					$get_request_book_intimate = 3;

					//處理積分
					rank($request_from,$get_request_from_rank,$rank_type=4,$conn_mssr,$arry_conn_mssr);
					rank($request_to,$get_request_to_rank,$rank_type=4,$conn_mssr,$arry_conn_mssr);

					//處理書友親密度
					friend_intimate($request_from, $request_to, $get_request_book_intimate, $intimate_type=1, $conn_mssr, $arry_conn_mssr);
...
if ($use_new_system) {
                    $msg .= "<br><br>你獲得獎勵<b>積分</b> " . $get_request_to_rank . " 分";
                    $msg .= "<br><br>你與" . $request_from_user_name . "之間的<b>親密度</b>增加了 " . $get_request_book_intimate . " 點";
}
                    die($msg);
            }
                    ...
                    if($friend_state===''){
                       $arry_err[]='交友狀態,未輸入!';
                        ...
                        if($friend_state===0){
                            ...
                            if(!in_array($friend_state,array(1,2,3))){
                                $arry_err[]='交友狀態,錯誤!';
                            }
                        }
                    }
	...
	//-----------------------------------------------
	//函式: check_report_message()
	//用途: 確認被檢舉訊息
	//-----------------------------------------------

		function check_report_message($send_url,$arrys_sess_login_info){
		//-------------------------------------------
		//函式: check_report_message()
		//用途: 確認被檢舉訊息
		//-------------------------------------------
		    ...
		    //---------------------------------------
		        ...
		        //POST
		        $message_id=trim($_POST[trim('message_id')]);
		    ...
		    //---------------------------------------
		        ...
		        $sql="
		            UPDATE `mssr_forum`.`mssr_forum_report_message` SET
		                `message_status`  = 2
		            WHERE 1=1
