...
<body>
    ...
    <div id="sp_help" style="position:absolute; top:236px; left:220px; color:#993300; font-size:36px; text-align:center; width: 568px; display:none">還沒訂閱嗎?<BR>可以到別人家看書訂閱喔!</div>
	...
	<div id="page_text" style="position:absolute; top:402px; left:422px; width:177px; height:47px; font-size: 36px; text-align: center; font-weight: bold; color: #406031;"></div>
    <script>
	//---------------------------------------------------
	//初始化
	//---------------------------------------------------
	var booking_max_count = 0;
	var page = 1 ;
	var max_page = 1 ; 
	//---------------------------------------------------
	//FUNCTION
	...
	*/
	...
	{
		echo("get_booking_count:初始開始:讀取訂閱筆數");
		cover("讀取販賣頁面")
		var url = "./ajax/get_booking_count.php";
		$.post(url, {
			...
			}).success(function (data) 
			{
				...
				if(data[0]!="{")
				{
					echo("AJAX:success:get_booking_count():讀取訂閱筆數:資料庫發生問題");
					cover("喔喔?! 讀取失敗了喔  請確認網路連",2,function(){get_booking_count();});
					return false;
				}
				
				data_array = JSON.parse(data);
				echo("AJAX:success:get_booking_count():讀取訂閱筆數:已讀出:"+data);
				if(data_array["error"]!="")
				{
				...
				{
					booking_max_count = data_array["booking_count"];
					if(booking_max_count == 0)
					{
						max_page = 1;
						window.document.getElementById("sp_help").style.display="block";	
					}
					else
					{max_page = Math.floor((Math.floor(booking_max_count)-1)/10)+1;}
				...
				echo("AJAX:error:get_booking_count():讀取訂閱筆數:");
				cover("喔喔?! 讀取失敗了喔  請確認網路連",2,function(){get_booking_count();});
			}).complete(function(e){
				echo("AJAX:complete:main():讀取訂閱筆數:");
			});
	}
