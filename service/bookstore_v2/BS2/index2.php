...
<!--==================================================
    ...
    <a id="home" style="position:absolute; top:333px; left:863px; cursor:pointer;" onClick="out(<? echo $_SESSION['uid'];?>)"></a>
...
<div id="debug" style="position:absolute;top:520px;"></div>
	...
	*/
		...
		$.post(url, {
			...
			}).success(function (data) 
			{
				...
				}else 
				{
					...
					if(game_data["group"]!="" && data_array["same_school"] == 1)
					{			
						set_peason_layer();
					...
					{
						if(data_array["midentity"]!= "t" )cover("喔喔，你還沒有被加入班級喔，請老師幫你加入班級吧",1);
						set_school_layer();
					}
				}
			}).error(function(e){
	...
	{	
		...
		if(game_data["semester_code"]!=""){
			...
			}).success(function (data) 
			{
					...
					}else
					{
						...
						//目前只開放自己學校
						window.document.getElementById("star").innerHTML = window.document.getElementById("star").innerHTML + '<div id="star_'+i+'" onClick="set_school_info(\''+game_data["school"]+'\',\''+game_data["semester_code"]+'\')" style="cursor:pointer; position:absolute; top:'+(50)+'px; left:'+(220)+'px;"> </div>';
		...
		if(1)
		{
			window.document.getElementById("star").innerHTML = window.document.getElementById("star").innerHTML + '<div id="star_s" onClick="set_credit_school_info()" style="cursor:pointer; position:absolute; top:'+(150)+'px; left:'+(360)+'px;"> </div>';
	...
	function set_school_info(value,value2)
	{
		...
		}).success(function (data) 
		{
				...
				}else
				{
					for(var i = 0 ; i <= data_array["count"] ; i++)
					{
						...
						window.document.getElementById("star_"+i).innerHTML = window.document.getElementById("star_"+i).innerHTML +'<div style="position:absolute; top:180px; left:32px; width: 234px; height: 32px;" class="world_bar2">'+data_array[i]["star_declaration"]+'</div>';
						if(data_array[i]["parent_id"]!="")
						{		
							window.document.getElementById("star").innerHTML = window.document.getElementById("star").innerHTML +'<a  onClick="out('+data_array[i]["parent_id"]+')"  class="sky_boll_s"  style="position:absolute; cursor:pointer; top:'+(90*i%300+140)+'px; left:'+(200*i+213)+'px;"></a>';
						}
	...
	function open_select()
	{
		...
		if(window.document.getElementById("select_page").style.display == "none")window.document.getElementById("select_page").style.display = "block";
		else window.document.getElementById("select_page").style.display = "none";
	}
