...
//-------------------------------------------------------
    ...
    //-----------------------------------------------
		...
		if($coin > 0)$retrun_data["text"]="錄音 推薦獎勵:".$coin;
            ...
            //-------------------------------------------
                ...
                $ftp_path="{$ftp_root}/{$user_id}/book/{$book_sid}";
                ...
                $_arrys_path=array(
                    ...
                    "{$ftp_path}/record"                       =>mb_convert_encoding("{$ftp_path}/record",$fso_enc,$page_enc)
                 );
	...
	foreach(array('video', 'audio') as $type) {
		if (isset($_FILES["${type}-blob"])) {
			...
			$hostname = "ftp://".$user . ":" . $pass . "@" . $host . "/public_html/mssr/info/user/{$user_id}/book/{$book_sid}/record"; 
			...
			//重新連接 | 重新登入 FTP
					...
					//設置ftp路徑
                    ftp_chdir($ftp_conn,"{$ftp_path}/record");

                    //ftp上傳
                    ftp_put($ftp_conn,"{$fileName}",$_FILES["${type}-blob"]["tmp_name"],FTP_BINARY);
