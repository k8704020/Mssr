...
<body>
        ...
        <section class="experiment recordrtc" style=" max-width:400px;">
            <h2 class="header">
            	<button id="sopbtn">點我錄音(第一個)<a style="color:#FF0000">●</a></button><BR>
			...
			<div style="text-align: center; display: none; width:350px;">
                <button id="save-to-disk" style=" display:none;">下載▼</button>
                <button id="open-new-tab" style=" display:none;">Open New Tab</button>
                <button id="upload-to-server">上傳第一個錄音檔案▲</button>
                <button id="upload-to-server2" style="display:none;">上傳第二個錄音檔案▲</button>
                <button id="out" onClick="out()" style="display:none; float:right;">完成離開</button>
                <BR>
            ...
            <div id="upbar" style="display: none; position:relative;background-color:#CCC;height:18px;  text-align: center; border-radius:10px; box-shadow:-1px -1px 1px #333">
                ...
                <div id="upbar_t" style=" position:absolute; top:0px;color:#FFFFFF; text-shadow: 1px 1px 1px #000; font-size:10px">上傳進度 : 60%</div>
            </div>
            <video controls muted style="display:none;"></video>
            <div id="videovideo" flag=1></div>
            <div id="videovideo2" flag=0></div>
        </section>
        ...
        <script>
            ...
            recordingDIV.querySelector('button').onclick = function() {
                ...
                if(button.innerHTML === '結束錄音<a style="color:#FF0000">■</a>') {
					close_timer();
                    ...
                    var ovideovideo1= document.querySelector('#videovideo');
                    var ovideovideo2= document.querySelector('#videovideo2');
                    if(ovideovideo1.getAttribute('flag')==0){
                        button.innerHTML = '點我錄音(第二個)<a style="color:#FF0000">●</a>';
                        var uploadserver2 = document.querySelector('#upload-to-server2');
                        uploadserver2.style.display="";
                    }
                    if(ovideovideo2.getAttribute('flag')==0){
                        button.innerHTML = '點我錄音(第一個)<a style="color:#FF0000">●</a>';
                        var uploadserver2 = document.querySelector('#upload-to-server2');
                        //uploadserver2.style.display="";
                    }
                    ...
                    if(button.recordRTC) {
                        if(button.recordRTC.length) {
                            ...
                            //console.log(button.recordRTC);
                            //console.log(456);
                            //record_blob1=document.querySelector('.recordrtc').querySelector('button').recordRTC.blob
                            button.recordRTC.stopRecording(function(url) {
                                ...
                                var ovideovideo1= document.querySelector('#videovideo');
                                var ovideovideo2= document.querySelector('#videovideo2');
                                //console.log(ovideovideo1.getAttribute('flag'));
                                //console.log(ovideovideo2.getAttribute('flag'));
                                if(ovideovideo1.getAttribute('flag')==0)record_blob1=button.recordRTC.blob;
                                if(ovideovideo2.getAttribute('flag')==0)record_blob2=button.recordRTC.blob;
                                //console.log(button.recordRTC.blob);
                                //console.log(record_blob1);
                                //console.log(456);
                            });
                        }
                    }
				...
				var commonConfig = {
                    onMediaCaptured: function(stream) {
                        ...
                        var uploadserver2 = document.querySelector('#upload-to-server2');
               			uploadserver.innerHTML = '上傳第一個錄音檔案▲';
						uploadserver.disabled = true;
                        uploadserver2.innerHTML = '上傳第二個錄音檔案▲';
						uploadserver2.disabled = true;
						var savedisk = document.querySelector('#save-to-disk');
						savedisk.disabled = true;
                        button.innerHTML = '結束錄音<a style="color:#FF0000">■</a>';
                        button.disabled = false;
                    },
                    onMediaStopped: function() {
                        ...
                        var ovideovideo1= document.querySelector('#videovideo');
                        var ovideovideo2= document.querySelector('#videovideo2');
                        if(ovideovideo1.getAttribute('flag')==0)button.innerHTML = '點我錄音(第二個)<a style="color:#FF0000">●</a>';
                        if(ovideovideo2.getAttribute('flag')==0)button.innerHTML = '點我錄音(第一個)<a style="color:#FF0000">●</a>';
                ...
                if(/*recordingMedia.value === 'record-audio'*/1) {
                    ...
                    button.mediaCapturedCallback = function() {
                        ...
                        button.recordRTC2 = RecordRTC(button.stream, {
                            type: 'audio',
                            bufferSize: typeof params.bufferSize == 'undefined' ? 0 : parseInt(params.bufferSize),
                            sampleRate: typeof params.sampleRate == 'undefined' ? 44100 : parseInt(params.sampleRate),
                            leftChannel: params.leftChannel || false,
                        ...
                        button.recordingEndedCallback = function(url) {
                            ...
                            var tttmp2= document.querySelector('#videovideo2');

                            if(tttmp.getAttribute('flag')==1){
                                audio.id="audio1"
                                tttmp.innerHTML = "";
                                tttmp.appendChild(document.createElement('hr'));
                                tttmp.innerHTML+="第一個錄音檔：";
                                tttmp.appendChild(audio);
                                audio.onended = function() {
                                    audio.pause();
                                    audio.src = URL.createObjectURL(button.recordRTC.blob);
                                };
                                tttmp.setAttribute('flag',0);
                                tttmp2.setAttribute('flag',1);
                            }else{
                                audio.id="audio2"
                                tttmp2.innerHTML = "";
                                tttmp2.appendChild(document.createElement('hr'));
                                tttmp2.innerHTML+="第二個錄音檔：";
                                tttmp2.appendChild(audio);
                                audio.onended = function() {
                                    audio.pause();
                                    //audio.src = URL.createObjectURL(button.recordRTC2.blob);
                                };
                                tttmp.setAttribute('flag',1);
                                tttmp2.setAttribute('flag',0);
                            }
                            console.log(button.recordRTC.blob);
                            //console.log(button.recordRTC2.blob);
                        };
...
/*
            ...
            function saveToDiskOrOpenNewTab(recordRTC) {
                ...
                recordingDIV.querySelector('#upload-to-server2').disabled = false;

                recordingDIV.querySelector('#upload-to-server').onclick = function() {
                    if(!recordRTC) return alert('No recording found.');
                    //this.disabled = true;
                    var button = this;
                    uploadToServer(record_blob1, function(progress, fileURL) {
                        if(progress === 'ended') {
                            button.disabled = true;
                ...
                recordingDIV.querySelector('#upload-to-server2').onclick = function() {
                    if(!recordRTC) return alert('No recording found.');
                    //this.disabled = true;
                    var button = this;
                    uploadToServer(record_blob2, function(progress, fileURL) {
                        if(progress === 'ended') {
                            button.disabled = true;
                            button.innerHTML = '上傳完成';//
							upbar(200);
            ...
            function uploadToServer(recordRTC, callback) {
				...
				var blob = recordRTC instanceof Blob ? recordRTC : record_blob1;
                var recordingDIV = document.querySelector('.recordrtc');

                //console.log(blob);
                //console.log(recordingDIV.querySelector('button').recordRTC.blob);
                //console.log(record_blob1);
                //var oaudio1= document.querySelector('#audio1');
                ////console.log(oaudio1.src);
                //return false;
			...
			function record_time(value)
			{
				...
				window.document.getElementById("record_time").innerHTML += "/50(秒)";
			}
