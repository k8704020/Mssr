...
//-------------------------------------------------------
	...
	require_once(str_repeat("../",2)."/inc/mssr_rec_book_draw_sid/code.php");
    ...
    //---------------------------------------------------
        ...
        //POST
       	$uid        =(isset($_POST['unique_id']))?(int)$_POST['unique_id']:0;
		$book_id    =(isset($_POST['book_id']))?mysql_prep($_POST['book_id']):0;
		$auth_coin_open = $_POST['auth_coin_open']=='yes'?true:false;

		if($uid == 0 || $book_id == NULL || $book_id == "")
		{
			$array["error"] ="錯誤!!  請重新登傳輸";
			die(json_encode($array,1));
		}
	//---------------------------------------------------
	//END   設定與引用
	//---------------------------------------------------		 
	function save_rec_draw($book_id,$uid,$time,$conn_mssr,$arry_conn_mssr,$auth_coin_open)
	{
		//回傳數值
		$array =array();
		$array["error"] = "";
		...
		if($retrun[0]['count']  >= 1)
		{
			...
			$sql = "UPDATE mssr_rec_book_cno
					SET rec_draw_cno = rec_draw_cno+1 ,
						keyin_mdate = '".$dadad."',
						`rec_state` = 1,
			...
			{		
				...
				$sql = $sql."UPDATE mssr_rec_book_cno_semester
						SET rec_draw_cno = rec_draw_cno+1 , keyin_mdate = '".$dadad."', `rec_state` = 1,`keyin_ip` = '".$_SERVER["REMOTE_ADDR"]."'
						WHERE book_sid = '".$book_id."'
						AND   user_id = '".$uid."' 
						;";
			}
		...
		//無給錢狀況
		else if((@$retrun_rec_reward[0]['rec_reward'] == "無" || count($retrun_rec_reward) == 0)&&$auth_coin_open)
		{
			//判斷是否到達給金錢標標準
			if($time > 15)//作畫時間大於15秒
			{
				...
				$sql = $sql."INSERT INTO `mssr`.`mssr_user_item_log` (
					...
					//獲得經驗值
					$exp_type="rec_draw";					
					set_score_exp($conn='',$exp_type,$exp_score,$uid,$arry_conn_mssr);
