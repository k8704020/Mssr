...
//-------------------------------------------------------
	...
	//-------------------------------------------
		...
		$sql = "SELECT rec_content,rec_state
			...
			WHERE  `mssr_rec_book_text_log`.`user_id` =  '".$user_id."'
			AND  `mssr_rec_book_text_log`.`book_sid` =  '".$book_sid."'
		...
		foreach($retrun as $key1=>$val1)
		{
		  if($val1['rec_state']== "顯示")
		  {
			  ...
			  $sql_tmp = "
			  			SELECT comment_content,comment_score,keyin_cdate
			  ...
			  if(count($retrun_tmp)>0)
			  {
			  		foreach($retrun_tmp as $key2=>$val2)
			        {
					  ...
					  $array[$key2]['rec_text_content'] = mysql_prep($val2['comment_content']);
					  $array[$key2]['rec_text_score'] = mysql_prep($val2['comment_score']);
