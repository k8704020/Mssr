...
//-------------------------------------------------------
    ...
    //---------------------------------------------------
		...
		$sell_item      =(isset($_POST['sell_item']))?mysql_prep($_POST['sell_item']):0;

 		//trim();//去空白
		if($user_permission != $_SESSION["permission"] || $user_id != $_SESSION["uid"] )
		{
			$array["error"] ="你違法進入了喔!!  請重新登入2";
	...
	//-------------------------------------------
		...
		$sql = "SELECT *
				FROM  `mssr_item`
				WHERE item_id = '$sell_item'
				";
		$retrun = db_result($conn_type='pdo',$conn_mssr,$sql,$arry_limit=array(0,1),$arry_conn_mssr);
		if(!count($retrun))
		{
			$array["error"] ="你違法進入了喔!!  請重新登入3";
			die(json_encode($array,1));
		}
		$shop_coin = (int)$retrun[0]["item_coin"];
		$shop_id = $retrun[0]["item_id"];
		...
		$sql = "UPDATE `mssr`.`mssr_user_info`
			SET
				`map_item` = '".$item_map."' ,
				`box_item` = '".$item_box."' ,
				`user_coin` = user_coin + ".($shop_coin/5)."
