	...
	if(isset($arrys_sess_login_info[0]['uid'])){
		...
		//-------------------------------	
			...
			//-------------------------------
				...
				$sql="
					SELECT `group_task_id`,`available_score`
					FROM `mssr_forum`.`dev_group_mission_slave` 
					WHERE `group_task_id`={$group_task_id[$x]}
				";
				$get_slave_score =db_result($conn_type='pdo',$conn_mssr,$sql,array(),$arry_conn_mssr);
				foreach($get_slave_score as $key => $get_slave_score){
					$step_slave_score[$x][$key]	= $get_slave_score['available_score'];//抓取主的任務的可獲得分數
				}
				/* echo "<pre>";
...
/* 				if($y==0){//針對自己的任務
				...
				//--------------------------------------------------
					...
					if($y==0){//自己的任務分數
					...
					}else if($y!=0){//在朋友的任務中貢獻的任務分數
						
						$sql="
							SELECT count(`reply_id`) as cno
							FROM `mssr_forum`.`mssr_forum_reply_like_log`
							WHERE `article_id`={$article_id[$x][$y]} and `user_id`={$my_friend[$y]}
						";
						$get_article_reply_like_cno =db_result($conn_type='pdo',$conn_mssr,$sql,array(),$arry_conn_mssr);
						if(isset($get_article_reply_like_cno[0]['cno'])){
							$like_cno[$x][$y] = $get_article_reply_like_cno[0]['cno'];
						}else{
							$like_cno[$x][$y] = 0;
						}						
						$sql="
						...
						$get_potential_score[$x][$y]=
						$like_cno[$x][$y]*$step_slave_score[$x][1]+
						$article_reply_cno[$x][$y]*$step_slave_score[$x][2];
					}
				//--------------------------------------------------
				//正在執行第三步驟
				//--------------------------------------------------
				...
				//--------------------------------------------------	
					...
					if($y==0){//自己的任務分數
						...
						$sql="
							...
							WHERE `group_task_id`={$group_task_id[$x]} and `view_uid`={$uid} and `accept_uid`={$my_friend[$y]}
						";
						$get_slave_atask_id = db_result($conn_type='pdo',$conn_mssr,$sql,array(),$arry_conn_mssr);
						if(isset($get_slave_atask_id[0]['slave_atask_id'])){//如果已有紀錄，則撈出該筆紀錄的id
							$slave_atask_id = $get_slave_atask_id[0]['slave_atask_id'];
							
							$sql="
								SELECT `slave_atask_id`,`score`,`like_number`
								FROM `mssr_forum`.`dev_slave_step2_like`
								WHERE `slave_atask_id`={$slave_atask_id}
							";	
							$get_slave_step2_like = db_result($conn_type='pdo',$conn_mssr,$sql,array(),$arry_conn_mssr);
							$score1 			= $get_slave_step2_like[0]['score'];
							$like_cno[$x][$y]	= $get_slave_step2_like[0]['like_number'];
							
							$sql="
								SELECT `slave_atask_id`,`score`,`reply_number`
								FROM `mssr_forum`.`dev_slave_step2_reply`
								WHERE `slave_atask_id`={$slave_atask_id}
							";
							$get_slave_step2_reply = db_result($conn_type='pdo',$conn_mssr,$sql,array(),$arry_conn_mssr);
							$score2 					= $get_slave_step2_reply[0]['score'];
							$article_reply_cno[$x][$y]	= $get_slave_step2_reply[0]['reply_number'];
							
							$sql="
								SELECT `slave_atask_id`,`score`
								FROM `mssr_forum`.`dev_slave_step2_reply_liked`
								WHERE `slave_atask_id`={$slave_atask_id}
							";							
							$get_slave_step2_reply_liked = db_result($conn_type='pdo',$conn_mssr,$sql,array(),$arry_conn_mssr);
							$score3 = $get_slave_step2_reply_liked[0]['score'];
							
							$get_potential_score[$x][$y] = $score1+$score2+$score3;
						}else{
							$like_cno[$x][$y] = 0;
							$article_reply_cno[$x][$y] = 0;
							$get_potential_score[$x][$y] = 0;
