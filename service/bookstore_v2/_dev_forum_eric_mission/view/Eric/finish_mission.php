	...
	if(isset($arrys_sess_login_info[0]['uid'])){
		...
		if(isset($_POST["deliver_uid"])){//自己的完成任務
			...
			$sql="
				...
				FROM `mssr_forum`.`dev_complete_mission_log` 
				WHERE `group_task_id`={$task_id} and `deliver_uid`={$uid}
			";	
			$get_deliver_cno =db_result($conn_type='pdo',$conn_mssr,$sql,array(0,1),$arry_conn_mssr);
			if(isset($get_deliver_cno[0]['deliver_cno'])){				
				$deliver_cno = $get_deliver_cno[0]['deliver_cno'];
			}else{
				$deliver_cno = 0;
			}
			//-------------------------------------------------------------------
			echo json_encode(array("article_id"=>$article_id,"article_title"=>$article_title,"like_cno"=>$like_cno,"reply_cno"=>$reply_cno,"total_score"=>$total_score,"finish_time"=>$finish_time,"step_content"=>$step_content,"deliver_cno"=>$deliver_cno));
			...
			$sql="
				SELECT count(`master_ctask_id`) as deliver_cno
				FROM `mssr_forum`.`dev_complete_mission_log` 
				WHERE `group_task_id`={$task_id} and `deliver_uid`={$friend_uid}
			";	
			$get_deliver_cno =db_result($conn_type='pdo',$conn_mssr,$sql,array(0,1),$arry_conn_mssr);
			if(isset($get_deliver_cno[0]['deliver_cno'])){				
				$deliver_cno = $get_deliver_cno[0]['deliver_cno'];
