...
//-------------------------------------------------------
	...
	if($auth_open_publish <= 1)
	{
		...
		SELECT 		
	  	...
	  	FROM (
			   SELECT
				    IFNULL((
				     SELECT `book_verified`
				     FROM `mssr_book_unverified`
				     WHERE `mssr_book_unverified`.`book_sid` = mssr_rec_book_cno.book_sid
				     LIMIT 1
				    ),0) AS `book_verified`,
		  	...
		  	WHERE 1=1
 			and(  mqry.`book_from` <> 1
		  	or  (mqry.`book_from`= 1 and mqry.`book_verified`= 1 ))
	...
	//==================================上架條件4:  未開放聊書,三項推薦(除了評星以外)才可上架==============================================================
	else if($auth_open_publish == 4)
	{
		$count=0;
		$sql = "
			SELECT 		
	...
	//==================================上架條件5:  開放聊書,四項推薦(除了評星以外)才可上架==============================================================
	else if($auth_open_publish == 5)
	{
		$count=0;
		$sql = "
		SELECT 		
	...
	";
		...
		foreach($retrun as $key1=>$val1)
		{
			...
			//搜尋書籍審核
				$book_verified = 1;
				if( mb_substr( $val1['book_sid'],0,3,"utf-8")== "mbu")
			...
			//判定有無聊書功能之後,上架條件分別做甚麼事情
				...
				// if($count >= 2  && $auth_open_publish == 1 ){
				...
				// }else if($count >2 && $auth_open_publish == 4){//若有開放聊書.評星以外必須做三項推薦auth_open_publish 4

				// 	$array[$key1]["open"] = 1;


				// }else if($count >3 && $auth_open_publish == 5){//若有開放聊書.評星以外四項推薦都必須有做=auth_open_publish 5
			...
			if($auth_open_publish != 3)
			{
				if($count >= 2  && $auth_open_publish == 1
					|| $auth_open_publish == 2 
					|| $count >2 && $auth_open_publish == 4 
					|| $count >3 && $auth_open_publish == 5  )
