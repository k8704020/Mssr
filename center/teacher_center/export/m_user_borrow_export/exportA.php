...
//-------------------------------------------------------
            ...
            //-------------------------------------------
                ...
                if($rowcount===0){
                    ...
                    while($arry_row=$result->fetch(PDO::FETCH_ASSOC)){
                        ...
                        $rs_grade         =(int)($arry_row['grade']);
                        $rs_classroom     =(int)($arry_row['classroom']);
                    }
                }
    ...
    //---------------------------------------------------
        ...
        //-----------------------------------------------
            ...
            $rs_semester_start=mysql_prep($rs_semester_start);
            $rs_semester_end  =mysql_prep($rs_semester_end  );
            $date             =date("Y-m-d");
                ...
                if($rowcount===0){
                    ...
                    foreach($arrys_book_result as $inx=>$arry_book_result){
                        ...
                        if(!empty($arry_book_infos)){
                            ...
                            if(trim($rs_book_isbn_10)!=='')$rs_book_library_code=$rs_book_isbn_10;
                            if(trim($rs_book_isbn_13)!=='')$rs_book_library_code=$rs_book_isbn_13;
                        }else{continue;}
                        if(trim($arry_book_result['book_library_code'])!=='')$rs_book_library_code=trim($arry_book_result['book_library_code']);
        ...
        //-----------------------------------------------
            ...
            if(!empty($arrys_success_info)){
                ...
                foreach($arry_excel_title_info as $key=>$val){
                    ...
                    //欄位值
                    if($val==='A1')$obj_success_excel->getActiveSheet(0)->setCellValue($val,trim('姓名                                                                       '));
                    if($val==='B1')$obj_success_excel->getActiveSheet(0)->setCellValue($val,trim('讀者類別(教師/學生/志工)                                                   '));
                    if($val==='C1')$obj_success_excel->getActiveSheet(0)->setCellValue($val,trim('讀者年級(國小請填1-6;國中請填7-9)                                          '));
                    if($val==='D1')$obj_success_excel->getActiveSheet(0)->setCellValue($val,trim('讀者單位(請填數字,例如101,312)                                             '));
                    if($val==='E1')$obj_success_excel->getActiveSheet(0)->setCellValue($val,trim('讀者座號(請填數字,例如01,23)                                               '));
                    if($val==='F1')$obj_success_excel->getActiveSheet(0)->setCellValue($val,trim('讀者性別(請填男或女)                                                       '));
                    if($val==='G1')$obj_success_excel->getActiveSheet(0)->setCellValue($val,trim('書籍登錄號                                                                 '));
                    if($val==='H1')$obj_success_excel->getActiveSheet(0)->setCellValue($val,trim('書籍名稱                                                                   '));
                    if($val==='I1')$obj_success_excel->getActiveSheet(0)->setCellValue($val,trim('分類號                                                                     '));
                    if($val==='J1')$obj_success_excel->getActiveSheet(0)->setCellValue($val,trim('學年(請填數字,例如101)                                                     '));
                    if($val==='K1')$obj_success_excel->getActiveSheet(0)->setCellValue($val,trim('學期(上學期請填1;下學期請填2)                                              '));
                    if($val==='L1')$obj_success_excel->getActiveSheet(0)->setCellValue($val,trim('書籍借出時間(請填西元格式,精準到分可為:201401011203 ;精準到日可為 20140101)'));
                    if($val==='M1')$obj_success_excel->getActiveSheet(0)->setCellValue($val,trim('書籍應還時間(請填西元格式,精準到分可為:201401011203 ;精準到日可為 20140101)'));
                    if($val==='N1')$obj_success_excel->getActiveSheet(0)->setCellValue($val,trim('書籍歸還時間(請填西元格式,精準到分可為:201401011203 ;精準到日可為 20140101)'));
                }
                ...
                foreach($arrys_success_info as $rs_row=>$arry_success_info){
                    ...
                    foreach($arry_success_info as $rs_cell=>$val){
                        //欄位值, 上下左右置中
                        if(trim($arry_success_info[$rs_cell])==='')continue;
                        $obj_success_excel->getActiveSheet(0)->getCellByColumnAndRow($rs_cell,$rs_row+2)->setValueExplicit("{$arry_success_info[$rs_cell]}",PHPExcel_Cell_DataType::TYPE_STRING);
                        $obj_success_excel->getActiveSheet(0)->getStyleByColumnAndRow($rs_cell,$rs_row+2)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                        $obj_success_excel->getActiveSheet(0)->getStyleByColumnAndRow($rs_cell,$rs_row+2)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
                    }
                }
            }
    ...
    //mssr_user_borrow_export_list
        ...
        //-----------------------------------------------
            ...
            $arry_save_excel_info=array(
                trim('mssr_user_borrow_export_list')
            );
        ...
        //-----------------------------------------------
            ...
            if(!empty($arrys_success_info)){
                ...
                $obj_success_writer->save("{$root}/file_download/mssr_user_borrow_export_list.xls");
            }
