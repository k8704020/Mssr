...
//-------------------------------------------------------
    ...
    //---------------------------------------------------
        ...
        //99    管理者
            ...
            if(in_array($auth_sys_check_lv,array(1,20,24,3,14,16,22,99))){
                if(!empty($sess_login_info)){
        ...
        //-----------------------------------------------
            ...
            if((isset($sess_school_code))&&(trim($sess_school_code)!=='')){
                ...
                ";
                $book_category_results=db_result($conn_type='pdo',$conn_mssr,$sql,array(0,1),$arry_conn_mssr);
                if(empty($book_category_results)){
                    $cat_code=cat_code(1,mb_internal_encoding());
                    $sql="
                        INSERT INTO `mssr`.`mssr_book_category`(
                            `create_by`,`edit_by`,`school_code`,`cat_id`,`cat_name`,`cat_code`,`cat1_id`,`cat2_id`,`cat3_id`,`cat_state`,`keyin_cdate`,`keyin_mdate`
                        )VALUES({$sess_user_id},{$sess_user_id},'{$sess_school_code}',NULL,'未分類','{$cat_code}','1','1','1','啟用',NOW(),NOW());
                    ";

                    $conn_mssr->exec($sql);
                    $cat_code=cat_code(2,mb_internal_encoding());
                    $sql="
                        INSERT INTO `mssr`.`mssr_book_category` (
                            `create_by`,`edit_by`,`school_code`,`cat_id`,`cat_name`,`cat_code`,`cat1_id`,`cat2_id`,`cat3_id`,`cat_state`,`keyin_cdate`,`keyin_mdate`
                        )VALUES({$sess_user_id},{$sess_user_id},'{$sess_school_code}',NULL,'生活','{$cat_code}','2','1','1','啟用',NOW(),NOW());
                    ";
                    $conn_mssr->exec($sql);
                    $cat_code=cat_code(3,mb_internal_encoding());
                    $sql="
                        INSERT INTO `mssr`.`mssr_book_category` (
                            `create_by`,`edit_by`,`school_code`,`cat_id`,`cat_name`,`cat_code`,`cat1_id`,`cat2_id`,`cat3_id`,`cat_state`,`keyin_cdate`,`keyin_mdate`
                        )VALUES({$sess_user_id},{$sess_user_id},'{$sess_school_code}',NULL,'藝術','{$cat_code}','3','1','1','啟用',NOW(),NOW());
                    ";
                    $conn_mssr->exec($sql);
                    $cat_code=cat_code(4,mb_internal_encoding());
                    $sql="
                        INSERT INTO `mssr`.`mssr_book_category` (
                            `create_by`,`edit_by`,`school_code`,`cat_id`,`cat_name`,`cat_code`,`cat1_id`,`cat2_id`,`cat3_id`,`cat_state`,`keyin_cdate`,`keyin_mdate`
                        )VALUES({$sess_user_id},{$sess_user_id},'{$sess_school_code}',NULL,'文學','{$cat_code}','4','1','1','啟用',NOW(),NOW());
                    ";
                    $conn_mssr->exec($sql);
                    $cat_code=cat_code(5,mb_internal_encoding());
                    $sql="
                        INSERT INTO `mssr`.`mssr_book_category` (
                            `create_by`,`edit_by`,`school_code`,`cat_id`,`cat_name`,`cat_code`,`cat1_id`,`cat2_id`,`cat3_id`,`cat_state`,`keyin_cdate`,`keyin_mdate`
                        )VALUES({$sess_user_id},{$sess_user_id},'{$sess_school_code}',NULL,'社會','{$cat_code}','5','1','1','啟用',NOW(),NOW());
                    ";
                    $conn_mssr->exec($sql);
                    $cat_code=cat_code(6,mb_internal_encoding());
                    $sql="
                        INSERT INTO `mssr`.`mssr_book_category` (
                            `create_by`,`edit_by`,`school_code`,`cat_id`,`cat_name`,`cat_code`,`cat1_id`,`cat2_id`,`cat3_id`,`cat_state`,`keyin_cdate`,`keyin_mdate`
                        )VALUES({$sess_user_id},{$sess_user_id},'{$sess_school_code}',NULL,'科學','{$cat_code}','6','1','1','啟用',NOW(),NOW());
                    ";
                    $conn_mssr->exec($sql);
                }
            }
    ...
    //---------------------------------------------------
        ...
        //-----------------------------------------------
            ...
            //進入許可檢核
            if(isset($auth_sys_arry_report[$sys_ename][$mod_ename]['access'])&&!$auth_sys_arry_report[$sys_ename][$mod_ename]['access']){
                //$url=str_repeat("../",5).'index.php';
                //header("Location: {$url}");
                //die();
            }
...
<div id="container">
    ...
    <div id="content">
                    ...
                    <td align="left" valign="middle" width="600px">
                        ...
                        <div id="navbar">
                            ...
                            <?php
                                ...
                                $sess_permission=addslashes($sess_permission);
                                $display="";
                                if(($sys_en==='forum')){
                                    if(!in_array('u_mssr_forum_tc',$arry_status)&&$auth_sys_check_lv!==99)$display="display:none;";
                                }
                            ?>
                                <li>
                    ...
                    <td align="left" valign="middle">
                        ...
                        <div id="q_form_class_code">
                            <ul id="q_form_class_code_ul">
                                <?php if(in_array($auth_sys_check_lv,array(1,20,24,3,14,16,22,99))):?>
        ...
        <div id="content_bottom">
            ...
            <div style='position:relative;top:30px;right:25px;float:right;font-family:"微軟正黑體","標楷體","新細明體";font-size:16px;font-weight:700;color:#87CDDC;border:0px solid #f00;display:none;'>
                過往的學期班級：
            ...
            <table class="teacher_datalist_tbl" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
                    <td>
                        ...
                        style="width:100%;height:855px;overflow:hidden;overflow-y:auto"></iframe>
                    </td>
                </tr>
            </table>
            <!-- 資料列表 結束 -->
...
//-------------------------------------------------------
    ...
    $(function(){
        ...
        <?php if(!empty($arrys_class_code_info)):?>
            var grade=0;
            <?php if(isset($arrys_class_code_rev['grade'][1])):?>
                var grade=parseInt(<?php echo $arrys_class_code_rev['grade'][1];?>);
            <?php endif;?>
            <?php if(isset($arrys_class_code_rev['grade'][2])):?>
                var grade=parseInt(<?php echo $arrys_class_code_rev['grade'][2];?>);
            <?php endif;?>
            for(key1 in jsons_class_code_rev){
                if(key1==='class_code'){
                    var json_class_code_rev=jsons_class_code_rev[key1];;
        ...
        //老師帶多班,預設選定一班
        <?php if(in_array($auth_sys_check_lv,array(22))):?>
            //var grade=<?php echo $arrys_class_code_rev['grade'][array_keys($arrys_class_code_rev['grade'])[1]]?>;
            //console.log(<?php echo $arrys_class_code_rev['grade'][array_keys($arrys_class_code_rev['grade'])[1]]?>);
            for(key in jsons_class_code_rev['class_code'][<?php echo $arrys_class_code_rev['grade'][array_keys($arrys_class_code_rev['grade'])[1]]?>]){
                //console.log(jsons_class_code_rev['class_code'][<?php echo $arrys_class_code_rev['grade'][array_keys($arrys_class_code_rev['grade'])[1]]?>][key]);
                q_form(jsons_class_code_rev['class_code'][<?php echo $arrys_class_code_rev['grade'][array_keys($arrys_class_code_rev['grade'])[1]]?>][key]);
                break;
            }
        <?php endif;?>
