...
//-------------------------------------------------------
    ...
    //99    管理者
        ...
        if(!empty($sess_login_info)){
            $auth_sys_check_lv=auth_sys_check($sess_login_info['responsibilities'],'m_class_rec_info');
        }
    ...
    //---------------------------------------------------
        ...
        if(isset($_SESSION['m_class_rec_info']['filter'])){
            $filter=$_SESSION['m_class_rec_info']['filter'];
            if(isset($_SESSION['m_class_rec_info']['class_code'])&&(trim($_SESSION['m_class_rec_info']['class_code'])!=='')){
                $q_class_code=mysql_prep(trim($_SESSION['m_class_rec_info']['class_code']));
            }
        }
        if(isset($_SESSION['m_class_rec_info']['query_fields'])){
            $query_fields=$_SESSION['m_class_rec_info']['query_fields'];
        }
        ...
        if(isset($_SESSION['m_class_rec_info']['class_code'])&&trim($_SESSION['m_class_rec_info']['class_code'])!==''){
            $_SESSION['teacher_center']['class_code']=trim($_SESSION['m_class_rec_info']['class_code']);
        }
                ...
                <div class="mod_data_tbl_outline" style="margin-top:35px;">
                    <table id="mod_data_tbl" border="0" width="95%" cellpadding="5" cellspacing="0" style="margin:30px 0;" class="table_style3">
                        ...
                        <tr align="center" valign="middle" class="fsize_18">
                            ...
                            <th width="300px" height="40px">推薦書本總數</th>
                            <th width="300px" height="40px">平均推薦書本</th>
                        </tr>
                        </thead>
                        ...
                        //---------------------------------------------------
                            ...
                            //-----------------------------------------------
                                ...
                                if(!empty($arry_student_uid)){
                                    ...
                                    $sql="
                                        SELECT `mssr`.`mssr_rec_book_cno_semester`.`user_id`
                                        FROM `mssr`.`mssr_rec_book_cno_semester`
                                        WHERE 1=1
                                            AND `mssr`.`mssr_rec_book_cno_semester`.`user_id` IN ($list_student_uid)
                                            AND `mssr`.`mssr_rec_book_cno_semester`.`keyin_cdate` BETWEEN '{$semester_start} 00:00:00' AND '{$semester_end} 23:59:59'
                                        GROUP BY `mssr`.`mssr_rec_book_cno_semester`.`user_id`, `mssr`.`mssr_rec_book_cno_semester`.`book_sid`
                                    ";
                                    $book_rec_cno_results=db_result($conn_type='pdo',$conn_mssr,$sql,array(),$arry_conn_mssr);
                                    $book_rec_cno=count($book_rec_cno_results);
                                }

                                $book_rec_avg=0;
                                if($student_cno!==0 && $book_rec_cno!==0){$book_rec_avg=round($book_rec_cno/$student_cno,1);}
                        ?>
                        <tr class="fsize_16">
                            ...
                            <td height="35px" align="center" valign="middle"><?php echo $book_rec_cno;?>本</td>
                            <td height="35px" align="center" valign="middle"><?php echo $book_rec_avg;?>本</td>
                        </tr>
                        <?php endforeach ;?>
                    </table>
                </div>
