...
//-------------------------------------------------------
    ...
    //type          類型
        ...
        $arr=array();
            ...
            //best_id
            if(isset($_GET['best_id'])&&(trim($_GET['best_id']!==''))){
                $best_id=trim($_GET['best_id']);
            }else{
                $best_id=0;
            }
            $arr['best_id']=array(
    ...
    //儲存條件式
    //---------------------------------------------------
    //$_SESSION['m_user_rec_book_best_class']['filter']        查詢條件式
    //$_SESSION['m_user_rec_book_best_class']['query_fields']  查詢欄位,顯示用
        ...
        $_SESSION['m_user_rec_book_best_class']['query_fields']=$query_fields;
        $_SESSION['m_user_rec_book_best_class']['filter']=$filter;
        if($school_code!=='')$_SESSION['m_user_rec_book_best_class']['school_code']=$school_code;
        $_SESSION['m_user_rec_book_best_class']['class_code']=$class_code;
        $_SESSION['m_user_rec_book_best_class']['best_id']=$best_id;

        if(1==2){//除錯用
            echo $_SESSION['m_user_rec_book_best_class']['filter'].'<p>';
            echo "<pre>";
            print_r($_SESSION['m_user_rec_book_best_class']['query_fields']);
            echo "</pre>";
            die();
        }
    ...
    //---------------------------------------------------
        ...
        if($type==='self'){
            ...
            $page=str_repeat("../",0)."show_content.php";
        }
