...
//-------------------------------------------------------
    ...
    //---------------------------------------------------
        ...
        //-----------------------------------------------
            ...
            $arrys_rec_book_best_class_code_info=[];
            $sql="
                ...
                FROM `mssr`.`mssr_rec_book_best_class`
                    ...
                    `mssr`.`mssr_rec_book_best_class`.`class_code`=`user`.`class`.`class_code`
                ...
                WHERE 1=1
                    ...
                    AND `user`.`class`.`class_category`=`user`.`class_name`.`class_category`
                GROUP BY `user`.`class`.`class_code`

                ORDER BY `user`.`semester`.`semester_year`, `user`.`semester`.`semester_term`,
                         `user`.`class`.`grade`, `user`.`class`.`classroom`
                ASC
            ";
            $arrys_rec_book_best_class_code_info=db_result($conn_type='pdo',$conn_mssr,$sql,array(),$arry_conn_mssr);
            //echo "<Pre>";
            //print_r($arrys_rec_book_best_class_code_info);
            //echo "</Pre>";
...
<div id="container">
    ...
    <div id="content">
            ...
            <table class="teacher_datalist_tbl" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
                    ...
                    <td align="left" valign="middle">
                        <!-- 查詢表單列 開始 -->
                        <div style="position:relative;float:left;width:195px;top:5px;border:0px solid #f00;display:none;">
                        ...
                        <div id="q_form_class_code" style="display:none;">
        ...
        <div id="content_bottom">
            ...
            <?php if(isset($arrys_rec_book_best_class_code_info)&&(!empty($arrys_rec_book_best_class_code_info))):?>
            <div style='position:relative;top:30px;right:25px;float:right;font-family:"微軟正黑體","標楷體","新細明體";font-size:16px;font-weight:700;color:#87CDDC;border:0px solid #f00;'>
                請選擇學期與班級：
                <select id="rec_book_best_class" name="rec_book_best_class" size="" style="border:1px solid #87CDDC;" onchange="sel_rec_book_best_class(this);void(0);">
                    <option value="">請下拉選擇
                    <?php foreach($arrys_rec_book_best_class_code_info as $arry_rec_book_best_class_code_info):?>
                    <?php
                        $rs_semester_year=(int)$arry_rec_book_best_class_code_info['semester_year'];
                        $rs_semester_term=(int)$arry_rec_book_best_class_code_info['semester_term'];
                        $rs_grade        =(int)$arry_rec_book_best_class_code_info['grade'];
                        $rs_class_name   =trim($arry_rec_book_best_class_code_info['class_name']);
                        $rs_class_code   =trim($arry_rec_book_best_class_code_info['class_code']);

                        $rs_semester_year=$rs_semester_year-1911;
                    ?>
                        <option value="<?php echo $rs_class_code;?>"><?php echo "{$rs_semester_year}年第{$rs_semester_term}學期{$rs_grade}年{$rs_class_name}班";?>
                    <?php endforeach;?>
                </select>
            </div>
            <?php endif;?>
...
//-------------------------------------------------------
    ...
    function sel_rec_book_best_class(obj){
        var class_code=trim(obj.value);
        if(class_code===''){
            alert('請選擇學期與班級');
            return false;
        }
