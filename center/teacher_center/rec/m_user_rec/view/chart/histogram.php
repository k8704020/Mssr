...
//-------------------------------------------------------
    ...
    //---------------------------------------------------
        ...
        $sql="
            SELECT `user_id`, `keyin_mdate`
            FROM `mssr_rec_book_cno`
            WHERE 1=1
                AND `user_id` IN ({$list_uid})
                AND `rec_state`=1
                AND `keyin_mdate` BETWEEN '{$get_filter_semester_start} 00:00:00' AND '{$get_filter_semester_end} 23:59:59'
        ...
        foreach($arrys_results as $arrys_result){
            ...
            $rs_keyin_mdate=trim($arrys_result['keyin_mdate']);
            $rs_keyin_mdate=date("Y-m",strtotime($rs_keyin_mdate));
            if(array_key_exists($rs_keyin_mdate,$arrys_month))$arrys_month[$rs_keyin_mdate]['group']=$arrys_month[$rs_keyin_mdate]['group']+1;
        }
?>
<!DOCTYPE HTML>
<Html>
