...
//-------------------------------------------------------
    ...
    //coin_open  書店葵幣開放條件
        ...
        //GET
        $coin_open=trim($_GET[trim('coin_open')]);
    ...
    //coin_open  書店葵幣開放條件
        ...
        if($coin_open===''){
           $arry_err[]='書店葵幣開放條件,未輸入!';
        }else{
            $coin_open=trim($coin_open);
            if(!in_array($coin_open,array("yes","no","all_no"))){
                $arry_err[]='書店葵幣開放條件,錯誤!';
            }
        }
...
//-------------------------------------------------------
    ...
    //---------------------------------------------------
        ...
        //設定, 書店葵幣開放條件
        $bookstore_coin_open=$arry_coin_open[trim($coin_open)];
?>
<!-- 內容 開始 -->
    ...
    <table align="center" border="0" width="700px" class="table_style0" style="position:relative;top:30px;">
        ...
        <tr class="fc_gray0">
            ...
            <td width="300px" height="150px" class="b_line gr_dashed">
                <?php foreach($arry_coin_open as $key=>$val):?>
                    <input type="radio" value="<?php echo $key;?>" name="coin_open"
                    <?php echo ($key===$coin_open)?'checked':'';?>>
                    <?php echo $val;?><br/><br/>
                <?php endforeach;?>
            </td>
        </tr>
