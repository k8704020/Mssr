...
//-------------------------------------------------------
    ...
    //99    管理者
        ...
        if(!empty($sess_login_info)){
            $auth_sys_check_lv=auth_sys_check($sess_login_info['responsibilities'],'m_borrow_limit_cno');
        }
...
//-------------------------------------------------------
    ...
    //---------------------------------------------------
        ...
        //設定, 閱讀登記數量限制
        $bookstore_borrow_limit_cno=(int)(@unserialize($arrys_result[0]['auth'])['borrow_limit_cno']);
?>
<!-- 條件資訊  開始 -->
...
<table cellpadding="0" cellspacing="0" border="0" width="100%" class="table_style2" style="position:relative;top:40px;"/>
    <tr align="left" valign="middle">
        <td width="100%" height="100px" align="center">
            班級閱讀登記數量限制：
            <select id="borrow_limit_cno" name="borrow_limit_cno" class="form_select" style="position:relative;"
            onchange="edit(this);void(0);">
                <?php for($i=3;$i<=20;$i++):?>
                    <option value="<?php echo $i;?>" <?php if($i===$bookstore_borrow_limit_cno)echo 'selected';?>>一天最多 <?php echo $i;?> 本
                <?php endfor;?>
            </select>
        </td>
    </tr>
...
//-------------------------------------------------------
    ...
    //修改條件
        ...
        var arg ={
            'psize':psize,
            'pinx' :pinx,
            'borrow_limit_cno':encodeURI(obj.value),
