...
//-------------------------------------------------------
    ...
    //class_code        班級代號
        ...
        //POST
        $borrow_limit_cno=(int)$_GET[trim('borrow_limit_cno')];
    ...
    //class_code        班級代號
        ...
        if($borrow_limit_cno===''){
           $arry_err[]='閱讀登記數量限制,未輸入!';
        }else{
           $borrow_limit_cno=(int)$borrow_limit_cno;
           if($borrow_limit_cno===0){
              $arry_err[]='閱讀登記數量限制,不為整數!';
           }
        }
        if($class_code===''){
           $arry_err[]='班級代號,未輸入!';
    ...
    //---------------------------------------------------
        ...
        //class_code        班級代號
            ...
            //-------------------------------------------
                ...
                $sql="
                    ...
                    WHERE 1=1
                        AND `mssr`.`mssr_auth_class`.`class_code`='{$class_code}'
                ";
        ...
        //-----------------------------------------------
            ...
            $auth['borrow_limit_cno']=(int)$borrow_limit_cno;
            $class_code              =addslashes(trim($class_code));
            $auth                    =serialize($auth);
