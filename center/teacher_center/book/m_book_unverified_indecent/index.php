...
//-------------------------------------------------------
    ...
    //---------------------------------------------------
        ...
        require_once(str_repeat("./",1).'filter.php');
    ...
    //---------------------------------------------------
        ...
        $filter_indecent = implode(",",$arry_filter_indecent);;
    ...
    //---------------------------------------------------
        ...
        //-----------------------------------------------
            ...
            if($filter!=''){
                $query_sql="
                    ...
                    WHERE 1=1
                        -- FILTER在此
                        {$filter}
                        #AND `book_name` REGEXP '{$filter_indecent}'
                ";
            }else{
                $query_sql="
                    SELECT
            ...
            if(!empty($arry_filter_indecent)){
                $query_sql.="
                    AND (
                ";
                foreach($arry_filter_indecent as $inx=>$filter_indecent){
                    //echo "<Pre>";print_r($filter_indecent);echo "</Pre>";
                    $query_sql.="
                        `book_name` LIKE '{$filter_indecent}%'
                    ";
                     if($inx!==count($arry_filter_indecent)-1)$query_sql.="OR";
                }
                $query_sql.="
                    )
                ";
            }
            $query_sql.="ORDER BY `keyin_cdate` DESC";
            //echo "<Pre>";print_r($query_sql);echo "</Pre>";
            //die();
                ...
                <tr>
                    ...
                    <td align="center" valign="middle">
                        <?php if($auth_sys_check_lv===99):?>
                            <select id="book_verified" name="book_verified" class="form_select" style="position:relative;height:23px;display:none;"
