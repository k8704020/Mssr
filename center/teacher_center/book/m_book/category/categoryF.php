        ...
        //-----------------------------------------------
            ...
            //-------------------------------------------
                ...
                if($numrow===0){
                    ...
                    $rs_book_name=trim($arrys_result[0]['book_name']);
                }
...
<form id='Form1' name='Form1' method='post' onsubmit="return false;">
    <table align="center" border="0" width="525px" class="table_style0" style="position:relative;top:30px;">
        ...
        <tr class="fc_gray0">
            <td align="right" width="80px" height="45px" class="b_line gr_dashed">
                書籍名稱：
            </td>
            <td height="45px" class="b_line gr_dashed">
        ...
        ?>
            ...
            ?>
            <tr class="fc_gray0">
                <td align="right" width="80px" height="45px" class="">
                    第<?php echo $con_num;?>種類別：
                </td>
                <td height="45px" class="">
        ...
        <?php else:?>
            <tr class="fc_gray0">
                <td align="center" height="60px" class="" colspan="2">
                    <span class="fc_red1">目前尚未設定類別 !</span>
                </td>
            </tr>
        <?php endif;?>
        <tr class="fc_gray0">
            <td colspan="2" align="center" height="80px" class="gr_dashed">
                <span class="fc_red1">*</span>
                新增類別：<span class="fc_blue0">請特別注意，新增類別之後，<span class="fc_red1">將會移除上面原有的類別 !</span></span>
            </td>
        </tr>
        <tr class="fc_gray0">
            <td colspan="2" align="center" class="gr_dashed">
                <div id="div_category_group">
                    <div name="category_group" cno="1" style="position:relative;margin:5px 0px;">
                    <span class="fc_blue0">第一階：</span>
                    <select id="cat1_id[]" name="cat1_id[]" class="form_select"
                    onchange="lv1_set(this.options[this.selectedIndex].value,this.getAttribute('cno'));" cno="1">
                    ...
                    <span class="fc_blue0">第二階：</span>
                    <select id="cat2_id[]" name="cat2_id[]" class="form_select" cno="1">
                        <option value="1" att='請選擇'>請選擇
                    </select>

                    <span class="fc_blue0">第三階：</span>
                    <select id="cat3_id[]" name="cat3_id[]" class="form_select" cno="1">
                        <option value="1" att='請選擇'>請選擇
                    </select>
                    <span class="fc_red1 fsize_18" style="position:absolute;right:35px;" onmouseover="this.style.cursor='pointer'" onclick="add_category();">
                        &nbsp;+
                    </span>
                    <span class="fc_red1 fsize_18" style="position:absolute;right:15px;" onmouseover="this.style.cursor='pointer'" onclick="del_category();">
                        &nbsp;<b>-</b>
                    </span>
                    </div>
                <div>
        ...
        <tr>
            <td align="right" colspan="2">
                ...
                <input type="button" id="BtnS" name="BtnS" value="新增" class="ibtn_gr6030" style="margin:10px 0px;" tabindex="1" onmouseover="this.style.cursor='pointer'">
...
//-------------------------------------------------------
    ...
    var json_arrys_lv1=<?php echo (json_encode($arrys_lv_info['arrys_lv1']));?>;
    var has_cat_code_lv=0;

    var oForm1=document.getElementById('Form1');    //表單
    var oBtnS =document.getElementById('BtnS');     //新增
    var oBtnB =document.getElementById('BtnB');     //返回
    var ocategory_groups=document.getElementsByName('category_group');
    var odiv_category_group=document.getElementById('div_category_group');

    var ocat1_ids=document.getElementsByName('cat1_id[]');
    var ocat2_ids=document.getElementsByName('cat2_id[]');
    var ocat3_ids=document.getElementsByName('cat3_id[]');
    var ohas_cat_code_lv=document.getElementById('has_cat_code_lv');

    oBtnS.onclick=function(){
    //新增

        for(var i=0;i<ocategory_groups.length;i++){
            cat1_id=parseInt(ocat1_ids[i].value);
            cat2_id=parseInt(ocat2_ids[i].value);
            cat3_id=parseInt(ocat3_ids[i].value);
            if(cat1_id===1){
                alert('請進行分類動作!');
                return false;
                break;
    ...
    //刪除類別清單組
        var ocategory_group=ocategory_groups[ocategory_groups.length-1];
        if(ocategory_groups.length>1){
            $(ocategory_group).remove();
        }else{
            //alert('至少須有一組!');
        }
    }
    ...
    //新增類別清單組
        ...
        var ocategory_group=ocategory_groups[ocategory_groups.length-1];
        cno=parseInt(ocategory_group.getAttribute('cno'))+1;
        var _html='';

        //輸出設置
        _html+='<div name="category_group" cno="'+(cno)+'" style="position:relative;margin:5px 0px;">';
            _html+='<span class="fc_blue0">第一階：</span>&nbsp;';
            _html+='<select id="cat1_id[]" name="cat1_id[]" class="form_select" onchange="lv1_set(this.options[this.selectedIndex].value,'+cno+');" cno="'+(cno)+'">';
                _html+='<option value="1" att="請選擇">請選擇';
                for(key in json_arrys_lv1){
                    var js_cat1_id=parseInt(json_arrys_lv1[key]['cat1_id']);
                    var js_cat_name=trim(json_arrys_lv1[key]['cat_name']);
                    _html+='<option value="'+js_cat1_id+'">'+js_cat_name+'';
                }
            _html+='</select>';

            _html+='&nbsp;<span class="fc_blue0">第二階：</span>&nbsp;';
            _html+='<select id="cat2_id[]" name="cat2_id[]" class="form_select"cno="'+(cno)+'">';
                _html+='<option value="1" att="請選擇">請選擇';
            _html+='</select>';

            _html+='&nbsp;<span class="fc_blue0">第三階：</span>&nbsp;';
            _html+='<select id="cat3_id[]" name="cat3_id[]" class="form_select"cno="'+(cno)+'">';
                _html+='<option value="1" att="請選擇">請選擇';
            _html+='</select>';
        _html+='</div>';

        //附加
        $(odiv_category_group).append(_html);
    }
    ...
    function lv1_set(cat1_id,cno){
    //設置第一階類別

        var cno     =parseInt(cno);
        var cat1_id =cat1_id;

        var ocat2_id=ocat2_ids[cno-1];
        var ocat3_id=ocat3_ids[cno-1];
        ...
        try{
            ...
            //附加切換函式
            $(ocat2_id)[0].setAttribute('onchange','lv2_set(this.options[this.selectedIndex].value,'+cno+');');
        }catch(err){
            return false;
        }
    }

    function lv2_set(cat2_id,cno){
    //設置第二階類別

        var cno     =parseInt(cno);
        var ocat1_id=ocat1_ids[cno-1];
        var ocat2_id=ocat2_ids[cno-1];
        var ocat3_id=ocat3_ids[cno-1];
