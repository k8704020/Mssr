...
//-------------------------------------------------------
    ...
    //---------------------------------------------------
        ...
        //book_sid          書籍識別碼
            ...
            //-------------------------------------------
                ...
                //---------------------------------------
                    ...
                    $get_book_info=get_book_info($conn_mssr,$book_sid,$array_filter=array('book_isbn_10','book_isbn_13'),$arry_conn_mssr);
                    if(count($get_book_info)===0){
                        $msg="書本不存在, 請重新輸入!";
                        $jscript_back="
                            <script>
                                alert('{$msg}');
                        ...
                        $book_isbn_10=trim($get_book_info[0]['book_isbn_10']);
                        $book_isbn_13=trim($get_book_info[0]['book_isbn_13']);
                    }
        ...
        //-----------------------------------------------
            ...
            if($book_isbn_10!=='')$book_nonumbering.=",'{$book_isbn_10}'";
            if($book_isbn_13!=='')$book_nonumbering.=",'{$book_isbn_13}'";
        ...
        //-----------------------------------------------
            ...
            if(count($arrys_result)===0){
                ...
                $sql.="
                    # for mssr_black_book
                    DELETE FROM `mssr_black_book`
                    WHERE 1=1
                        AND `book_nonumbering`= '{$book_sid}'
                ...
                if($book_isbn_10!==''){
                    $sql.="
                        ...
                        DELETE FROM `mssr_black_book`
                        WHERE 1=1
                            AND `book_nonumbering`= '{$book_isbn_10}'
                            AND `create_by`={$sess_user_id}
                        LIMIT 1;
                    ";
                }
                if($book_isbn_13!==''){
                    $sql.="
                        ...
                        DELETE FROM `mssr_black_book`
                        WHERE 1=1
                            AND `book_nonumbering`= '{$book_isbn_13}'
                            AND `create_by`={$sess_user_id}
                        LIMIT 1;
                    ";
                }
