...
//-------------------------------------------------------
    ...
    //book_library_code 圖書館編碼
        ...
        //POST
        $arry_book_name          =array_map("trim",($_POST[trim('book_name        ')]));
        $arry_book_author        =array_map("trim",($_POST[trim('book_author      ')]));
        $arry_book_publisher     =array_map("trim",($_POST[trim('book_publisher   ')]));
        $arry_book_library_code  =array_map("trim",($_POST[trim('book_library_code')]));
    ...
    //book_library_code 圖書館編碼
        ...
        foreach($arry_book_name as $book_name){
            if($book_name===''){
               $arry_err[]='書籍名稱,未輸入!';
            }
        }

        foreach($arry_book_library_code as $book_library_code){
    ...
    //---------------------------------------------------
        ...
        //book_library_code 圖書館編碼
            ...
            $arry_book_name          =array_map("mysql_prep",$arry_book_name        );
            $arry_book_author        =array_map("mysql_prep",$arry_book_author      );
            $arry_book_publisher     =array_map("mysql_prep",$arry_book_publisher   );
            $arry_book_library_code  =array_map("mysql_prep",$arry_book_library_code);
            $sess_user_id            =(int)$sess_user_id;
            $cno                     =count($arry_book_library_code);
        ...
        //-----------------------------------------------
            ...
            $cno                =(int)$cno;
            $create_by          =(int)$sess_user_id+1;
            $edit_by            =(int)$sess_user_id+1;
        ...
        //-----------------------------------------------
            ...
            for($i=0;$i<$cno;$i++){
                ...
                $book_library_code  =mysql_prep(strip_tags($arry_book_library_code[$i]));
                $book_name          =mysql_prep(strip_tags($arry_book_name[$i]));
                $book_author        =mysql_prep(strip_tags($arry_book_author[$i]));
                $book_publisher     =mysql_prep(strip_tags($arry_book_publisher[$i]));
            ...
            <table border="0" width="100%" cellpadding="5" cellspacing="0" style="position:relative;top:50px;" class="table_style2">
                ...
                <tr>
                    <td height="350px" align="center" valign="middle">
                        ...
                        書籍已新增成功!
                        <!-- 名稱： -->
                        <!-- <span class="fc_blue0"><?php echo htmlspecialchars($book_name);?></span> --><br/><br/>
...
//-------------------------------------------------------
    ...
    var oBtnB =document.getElementById('BtnB');     //繼續新增
