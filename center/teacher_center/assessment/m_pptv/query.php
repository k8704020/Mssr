...
//-------------------------------------------------------
    ...
    //儲存條件式
    //---------------------------------------------------
    //$_SESSION['m_pptv']['filter']        查詢條件式
    //$_SESSION['m_pptv']['query_fields']  查詢欄位,顯示用
        ...
        $_SESSION['m_pptv']['query_fields']=$query_fields;
        $_SESSION['m_pptv']['filter']=$filter;
        if($school_code!=='')$_SESSION['m_pptv']['school_code']=$school_code;
        $_SESSION['m_pptv']['class_code']=$class_code;

        if(1==2){//除錯用
            echo $_SESSION['m_pptv']['filter'].'<p>';
            echo "<pre>";
            print_r($_SESSION['m_pptv']['query_fields']);
            echo "</pre>";
            die();
        }
    ...
    //---------------------------------------------------
        ...
        if($type==='self'){
            ...
            //目標班級
            $goal_class_code='';
            if((isset($_SESSION['m_pptv']['class_code']))&&(trim($_SESSION['m_pptv']['class_code'])!=='')){
                $goal_class_code=trim($_SESSION['m_pptv']['class_code']);
                $page=str_repeat("../",0)."/mssr/service/pptv/mange.php?class_code={$goal_class_code}";
            }else{
                $page=str_repeat("../",0)."/mssr/service/pptv/mange.php";
            }
        }
