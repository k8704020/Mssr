...
//-------------------------------------------------------
    ...
    //99    管理者
        ...
        if(!empty($sess_login_info)){
            $auth_sys_check_lv=auth_sys_check($sess_login_info['responsibilities'],'m_class_forum_info');
        }
    ...
    //---------------------------------------------------
        ...
        if(isset($_SESSION['m_class_forum_info']['filter'])){
            $filter=$_SESSION['m_class_forum_info']['filter'];
            if(isset($_SESSION['m_class_forum_info']['class_code'])&&(trim($_SESSION['m_class_forum_info']['class_code'])!=='')){
                $q_class_code=mysql_prep(trim($_SESSION['m_class_forum_info']['class_code']));
            }
        }
        if(isset($_SESSION['m_class_forum_info']['query_fields'])){
            $query_fields=$_SESSION['m_class_forum_info']['query_fields'];
        }
        ...
        if(isset($_SESSION['m_class_forum_info']['class_code'])&&trim($_SESSION['m_class_forum_info']['class_code'])!==''){
            $_SESSION['teacher_center']['class_code']=trim($_SESSION['m_class_forum_info']['class_code']);
        }
                ...
                <div class="mod_data_tbl_outline" style="margin-top:35px;">
                    <table id="mod_data_tbl" border="0" width="95%" cellpadding="5" cellspacing="0" style="margin:30px 0;" class="table_style3">
                        ...
                        <tr align="center" valign="middle" class="fsize_18">
                            ...
                            <th width="300px" height="40px">發文        </th>
                            <th width="300px" height="40px">回文        </th>
                        </tr>
                        </thead>
                            ...
                            //-----------------------------------------------
                                ...
                                if(!empty($arry_student_uid)){
                                    ...
                                    $sql="
                                        ...
                                        WHERE 1=1
                                            AND `mssr_forum`.`mssr_forum_article`.`user_id` IN ($list_student_uid)
                                            AND `mssr_forum`.`mssr_forum_article`.`keyin_cdate` BETWEEN '{$semester_start} 00:00:00' AND '{$semester_end} 23:59:59'
                                            AND `mssr_forum`.`mssr_forum_article`.`article_state`=1
                                    ";
                                    $article_cno_results=db_result($conn_type='pdo',$conn_mssr,$sql,array(),$arry_conn_mssr);
                                    $article_cno=count($article_cno_results);
                                }

                                $article_avg=0;
                                if($student_cno!==0 && $article_cno!==0){
                                    $article_avg=round($article_cno/$student_cno,2);
                                }
                            ...
                            //-----------------------------------------------
                                ...
                                if(!empty($arry_student_uid)){
                                    ...
                                    $sql="
                                        ...
                                        WHERE 1=1
                                            AND `mssr_forum`.`mssr_forum_reply`.`user_id` IN ($list_student_uid)
                                            AND `mssr_forum`.`mssr_forum_reply`.`keyin_cdate` BETWEEN '{$semester_start} 00:00:00' AND '{$semester_end} 23:59:59'
                                            AND `mssr_forum`.`mssr_forum_reply`.`reply_state`=1
                                    ";
                                    $reply_cno_results=db_result($conn_type='pdo',$conn_mssr,$sql,array(),$arry_conn_mssr);
                                    $reply_cno=count($reply_cno_results);
                                }

                                $reply_avg=0;
                                if($student_cno!==0 && $reply_cno!==0){
                                    $reply_avg=round($reply_cno/$student_cno,2);
                                }
                        ?>
                        <tr class="fsize_16">
                            ...
                            <td height="35px" align="center" valign="middle">總數：<?php echo $article_cno;?>&nbsp;&nbsp;平均：<?php echo $article_avg;?></td>
                            <td height="35px" align="center" valign="middle">總數：<?php echo $reply_cno;?>&nbsp;&nbsp;平均：<?php echo $reply_avg;?></td>
                        </tr>
                        <?php endforeach ;?>
                    </table>
                </div>
