...
// print_r($sess_login_info);
    ...
    //接收參數
    //---------------------------------------------------
    if(isset($_SESSION['m_forum_hot_booklist']['class_code'])&&(trim($_SESSION['m_forum_hot_booklist']['class_code'])!=='')){
                $q_class_code=mysql_prep(trim($_SESSION['m_forum_hot_booklist']['class_code']));
    }

    if(!empty($sess_login_info['arrys_class_code'][0]['class_code'])){
        $class_code  = $sess_login_info['arrys_class_code'][0]['class_code'];
        //$school_code = $sess_login_info['school_code'];
        ...
        if(empty($q_class_code)){
            ...
            //$school_code = $_SESSION['m_forum_hot_booklist']['school_code'];
        }
    }

        if(isset($_SESSION['m_forum_hot_booklist']['class_code'])&&trim($_SESSION['m_forum_hot_booklist']['class_code'])!==''){
            $_SESSION['teacher_center']['class_code']=trim($_SESSION['m_forum_hot_booklist']['class_code']);
        }
                    ...
                    <table id="mod_data_tbl" border="0" width="99%" height="40px" cellpadding="5" cellspacing="0" style="" class="font-weight1 font-family1 fc_green0">
                        <tr align="center" valign="middle" class="fsize_16">
                            <td align="center" valign="middle" width="650px">
                               <table border="0" width="99%" height="40px" >
                                   <tr>
                                       <td width="300px"></td>
                                       <td align="center" width="700px">
                                            <span class="fsize_16"> ●本頁資料計算時間為<span class="fc_red1"><?php echo htmlspecialchars($week_sdate);?></span>
                                                 ~
                                            <span class="fc_red1"><?php echo htmlspecialchars($week_edate);?></span>為止</span >
                                       </td>
                                       <td width="300px" align="right">
                                            <input type="button" value="觀看累計資料" class="ibtn_gr9030" onclick="booklist ('<?php echo $class_code ?>')" onmouseover="this.style.cursor='pointer'" style="cursor: pointer;">
                                            <input type="button" value="老師討論歷程" class="ibtn_gr9030" onclick="booklist1('<?php echo $class_code ?>')" onmouseover="this.style.cursor='pointer'" style="cursor: pointer;">
                                       </td>
                                   </tr>
                               </table>
                            </td>
                    ...
                    <table  id="mod_data_tbl" border="0" width="95%" cellpadding="5" cellspacing="0" style="margin:30px; 0" class="table_style3">
                        <thead>
                            <tr align="center" valign="middle" class="fsize_18">
                                <th>名次</th>
                                <th>書名</th>
                                <th width="100px">投票人數</th>
                                <th width="100px">討論次數</th>
                                <th>操作</th>

                            </tr>
                        </thead>
                        <?php $i = 0;?>
                        <?php  foreach($hotList as $key=>$value){ $i++; $sid = $value['book_sid'];?>
                               <tr align="center">
                                    <td>No.<?php echo $i ?>              </td>
                                    <td >
                                        <?php echo $value['book_name']?>
                                    </td>
                                    <td><?php echo $value['cno']?>人     </td>
                                    <td><?php echo $value['count']?>次  </td>
                                    <td>
                                           <input type="button"  value="討論"  name="box" sid ="<?php echo $sid ?>" tid ="<?php echo $teacher_uid ?>">
                                    </td>
                                </tr>
                        <?php } ?>
...
$(function() {
    ...
    function booklist(class_code) {

        var url = "book_list_log/index.php?class_code="+ class_code;
        window.open(url);
    }


    function booklist1(class_code) {

        var url = "book_list/index.php?class_code="+ class_code;
        window.open(url);
    }

    // function updateList(book_sid){

    //         var check  = $("input[name='box']").is( ':checked' );
    //         alert(check);
    //         if(check){
    //             //alert(book_sid);
    //             //location.href = "act/hotList/hotlist.php?book_sid=" + book_sid;
    //         }else{
    //             //location.href = "act/hotList/hotlist.php?book_sid=" + book_sid;
    //         }
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" align="center">
        <tr>
            ...
            <!-- 在此設定寬高 -->
                ...
                <!-- 內容 -->
                <table border="0" width="100%" cellpadding="5" cellspacing="0" style="position:relative;top:20px;" class="mod_data_tbl_outline">
