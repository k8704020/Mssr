                ...
                <div class="mod_data_tbl_outline" style="margin-top:35px;">
                    <table id="mod_data_tbl" border="0" width="95%" cellpadding="5" cellspacing="0" style="margin-top:30px;" class="table_style3">
                        <tr align="center" valign="middle" class="fsize_18">
                            <th width="575px" height="40px">文章標題節錄</th>
                        ...
                        //---------------------------------------------------
                            ...
                            if(mb_strlen($rs_article_title)>30){
                                $rs_article_title=mb_substr($rs_article_title,0,30)."..";
                            }

                            //文章狀態
                            $rs_article_state_code=1;
                            $rs_article_state =trim($rs_article_state);
                            if($rs_article_state!=='刪除'){
                                $rs_article_state_code=2;
                            }
                            ...
                            //-----------------------------------------------
                                ...
                                $sql="
                                    ...
                                    WHERE 1=1
                                        ...
                                        AND `mssr_forum_article_like_log`.`article_id`={$rs_article_id}
                                ";
                                $db_results=db_result($conn_type='pdo',$conn_mssr,$sql,array(0,1),$arry_conn_mssr);
                                if(!empty($db_results)){
                                    $rs_article_like_code=2;
                                }
                        ?>
                        <tr class="fsize_16">
                            ...
                            onclick="view_article_list(<?php echo $rs_article_id;?>,0);void(0);">
                            ...
                            <td height="30px" align="center" valign="middle">
                                <input type="button" value="<?php if($rs_article_like_code===1){echo '讚';}else{echo '收回讚';}?>"
                                class="ibtn_gr6030 <?php if($rs_article_like_code!==1)echo 'fc_blue0';?>" onmouseover="this.style.cursor='pointer'"
                                onclick="like(<?php echo (int)$user_id;?>,<?php echo $rs_article_id;?>,<?php echo (int)$rs_article_like_code;?>);void(0);">

                                <input type="button" value="<?php if($rs_article_state==='刪除'){echo '不隱藏';}else{echo '隱藏';}?>"
                                class="ibtn_gr6030 <?php if($rs_article_state==='刪除')echo 'fc_red0';?>" onmouseover="this.style.cursor='pointer'"
                                onclick="del(<?php echo (int)$user_id;?>,<?php echo $rs_article_id;?>,<?php echo (int)$rs_article_state_code;?>);void(0);">
                            </td>
                        </tr>
                        <?php endforeach ;?>
                    </table>
...
//-------------------------------------------------------
    ...
    function like(user_id,article_id,like_code){
    //讚
        var url ='';
    ...
    function del(user_id,article_id,del_code){
    //隱藏文章
        var url ='';
