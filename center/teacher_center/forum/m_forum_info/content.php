...
// print_r($_SESSION);
    ...
    //---------------------------------------------------
        ...
        if(isset($_GET['q_class_code'])&&trim($_GET['q_class_code'])!==''){
            $class_code=trim($_GET['q_class_code']);
        }
            ...
            <!-- 內容 -->
                    ...
                    <table id="mod_data_tbl" border="0" width="95%" cellpadding="5" cellspacing="0" style="margin:30px; 0" class="table_style3">
                        <thead>
                            <tr align="center" valign="middle" class="fsize_18">
                                <th rowspan="2">座號</th>
                                <th rowspan="2">姓名</th>
                                <th  rowspan="2"><div style="">篇數</div><div style=""><input type="button" class="" value="詳細" onclick="user_article('<?php echo $class_code ?>')"></div></th>
                                <th rowspan="2"><div style="">字數</div><div style=""><input  type="button" class="" value="詳細" onclick="user_word('<?php echo $class_code ?>')"></div></th>
                                <th rowspan="2"><div style="">互動</div><div style=""><input  type="button" class="" value="詳細" onclick="user_assess('<?php echo $class_code ?>')"></div></th>
                ...
                <?php foreach($students as $student_k =>$student_v){ ?>
                        <tr class="fsize_16" align="center">
                            <td>
                                <?php echo (int)$student_v['number']; ?>
                            </td>
                            <td>
                                <?php echo htmlspecialchars(trim($student_v['name']));   ?>
                            </td>
                             <td>
                                <?php echo (int)$forum_po_cno[$student_k]+$book_po_cno[$student_k]+$forum_repo_cno[$student_k]+$book_repo_cno[$student_k];?>篇
                            </td>

                            <td>
                                <?php echo (int)$avg_po_cno[$student_k]+$avg_repo_cno[$student_k];  ?>字
                            </td>

                            <td>
                                <?php echo (int)$Actively[$student_k];    ?>
                            </td>
                            <td>
                                <?php echo  (int)$like[$student_k]."讚:".(int)$report[$student_k]."檢舉";     ?>
                            </td>
                            <td>
                                <?php
                                    if($like_look[$student_k]==='')$like_look[$student_k]='無';
                                	if(strlen($like_look[$student_k])>25){
                                		echo  mb_substr($like_look[$student_k],0,8,"utf-8").'....';
                                	}else{
                                		echo $like_look[$student_k];
                                	}
                                ?>
                            </td>
                        </tr>
