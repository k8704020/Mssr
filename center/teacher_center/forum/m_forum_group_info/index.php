...
//建立連線 user
    ...
    if(!in_array($class_code,$arr_classCode)){
        $msg = call_alert("您沒有權限進入，請洽詢明日星球團隊人員");
        die($msg);
    }
            ...
            SELECT
                ...
                FROM
                    `mssr_article_forum_rev`
                       ...
                       `mssr_article_forum_rev`.`article_id`=`mssr_forum_article`.`article_id`
                WHERE 1=1
                    AND forum_id = sqry.forum_id
                    AND `mssr_forum_article`.`user_id` IN ($users)
                GROUP BY forum_id),0) as 'fawon',
                ...
                WHERE 1=1
                    AND forum_id = sqry.forum_id
                    AND `mssr_forum_article`.`user_id` IN ($users)
                GROUP BY forum_id),0)+IFNULL((SELECT
                    count(*)
                FROM
                    `mssr_article_reply_forum_rev`
                        LEFT JOIN `mssr_forum_article_reply` ON
                ...
                WHERE 1=1
                    AND forum_id = sqry.forum_id
                    AND `mssr_forum_article_reply`.`user_id` IN ($users)
                GROUP BY forum_id),0)) as 'ta'
            FROM
                mssr.mssr_forum as sqry
                where 1 = 1
                    and forum_id in(SELECT `forum_id` FROM `mssr_forum` WHERE `create_by` in($users))
                    and forum_state not in('停用')
                    order by ta desc
...
//討論人數 發文
                    $sql="
                            ...
                            FROM `mssr_article_forum_rev`
                                LEFT JOIN `mssr_forum_article` ON
                                `mssr_article_forum_rev`.`article_id`=`mssr_forum_article`.`article_id`
                            WHERE 1=1
                                AND `mssr_forum_article`.`user_id` IN ($users)
                                AND `mssr_article_forum_rev`.`forum_id`= '$rs_forum_id'
                    ";
    ...
    //討論人數 回文
                    $sql="
                            ...
                            WHERE 1=1
                                AND `mssr_forum_article_reply`.`user_id` IN ($users)
                                AND `mssr_article_reply_forum_rev`.`forum_id`='$rs_forum_id'
                     ";
...
<?php
            ...
            ";
            $arr_create_by = db_result($conn_type='pdo',$conn_mssr,$sql,array(),$arry_conn_mssr);
            $createname = $arr_create_by[0]['create_by'];


            $get_user_info=get_user_info($conn_user,$createname,$array_filter=array('name'),$arry_conn_user);
            $createName = $get_user_info[0]['name'];
?>

            <tr  class="fsize_16" align="center" valign="middle">
            <td height="35px" class="edit" id='<?php echo $forum_id?>'><?php echo $forum_name;?>  <a style="" href="#" class="dis">編輯</a></td>
            <td height="35px"><?php echo $createName     ?></td>
            <td height="35px"><?php echo $v['rs_fawon'];       ?></td>
            <td height="35px"><?php echo $v['rs_hawon'];       ?></td>
            <td height="35px"><?php echo $v['rs_cno'];         ?></td>
            <td height="35px"><button class = "close" value ="<?php echo $forum_id; ?>">刪除</button></td>
            <td><input  type="button" value="前往此小組" onclick="window.open('../../../../service/forum/mssr_forum_group_discussion.php?forum_id=<?php echo $forum_id;?>')"></td>
        </tr>
...
var closeGroup = $('.close')

closeGroup.on("click",function(){

    if(confirm('您確定要刪除')){
        window.location.href = 'doAct/doAct.php?forum_id='+$(this).val();
    }else{
        return false;
    }
})
