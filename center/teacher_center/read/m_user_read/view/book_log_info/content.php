...
//-------------------------------------------------------
    ...
    //---------------------------------------------------
         ...
         $name_sql_result=db_result($conn_type='pdo',$conn_user,$name_sql,array(),$arry_conn_user);
         $name=$name_sql_result[0]['name'];
    ...
    //---------------------------------------------------
       //處理時間
        ...
        foreach($arrys_results as $arrys_result){
            ...
            //學生每個月的橋樑書的比例
             $arrys_month[$rs_borrow_sdate]['bridge_book_rate']=round(($arrys_month[$rs_borrow_sdate]['bridge_book']/$arrys_month[$rs_borrow_sdate]['total_book'])*100,2);
            
            //學生每個月的文字書書的比例
            $arrys_month[$rs_borrow_sdate]['words_book_rate']=round(($arrys_month[$rs_borrow_sdate]['words_book']/$arrys_month[$rs_borrow_sdate]['total_book'])*100,2);
        ...
        $about_book_level=[];
        $about_book_level['book_category']['picture_book']=0;
        $about_book_level['book_category']['bridge_book']=0;
        $about_book_level['book_category']['words_book']=0;
        ...
        $book_log_total=count($arrys_results);
        ...
        foreach($arrys_results as $arrys_result){
            ...
            $sql1="
                SELECT `mssr_idc_book_sticker_level_info`.`administrator_level`,
                       `mssr_idc_book_sticker_level_info`.`book_sid` 
                FROM `mssr_idc_book_sticker_level_info` 
                WHERE 1=1
                      AND `mssr_idc_book_sticker_level_info`.`book_sid`='{$arrys_result['book_sid']}'
                GROUP BY `mssr_idc_book_sticker_level_info`.`book_sid`
            ";

            $arrys_results1=db_result($conn_type='pdo',$conn_mssr,$sql1,array(),$arry_conn_mssr);

            foreach ($arrys_results1 as $key => $arrys_book) {


                if($arrys_book['administrator_level']<3&&$arrys_book['administrator_level']>0){

                    $about_book_level['book_category']['picture_book']=$about_book_level['book_category']['picture_book']+1;

                }else if($arrys_book['administrator_level']>2&&$arrys_book['administrator_level']<5){

                    $about_book_level['book_category']['bridge_book']=$about_book_level['book_category']['bridge_book']+1;

                }else{

                    $about_book_level['book_category']['words_book']=$about_book_level['book_category']['words_book']+1;
...
<Body>
        <div class="mod_data_tbl_outline" style="margin-top:35px;">
            ...
            <div class="header"><span><?php echo $name ?>登記書籍狀況</span></div>
            <div class="time"><span >統計時間:<?php echo $get_filter_semester_start,"-",$get_filter_semester_end; ?></span></div>

   <div class="title">  <p>每月學生登記橋梁書狀況</p><div id="month_about_book" style="width: 1000px; height: 400px;"></div></div>
    <div class="title"> <p >每週學生登記橋梁書狀況</p><div id="one_week_about_book" style="width:3595px; height: 500px;"></div></div>
    <div class="title"> <p>雙週學生登記橋梁書狀況</p><div id="two_week_about_book" style="width: 2080px; height: 500px; "></div></div>
    
    <div id="container1" style="max-width: 800px; height: 300px; margin: 0 auto"></div>
     <!--  <div id="container2" style="max-width: 800px; height: 300px; margin: 0 auto"></div> -->
...
//柱狀圖&折線圖
    ...
    var total= <?php echo $book_log_total;?>;
    var about_picture_book= <?php echo $about_book_level['book_category']['picture_book']; ?>;
    var about_bridge_book=<?php  echo $about_book_level['book_category']['bridge_book']; ?>;
    var about_words_book=<?php  echo $about_book_level['book_category']['words_book']; ?>;
   ...
   <?php endforeach;?>
    ...
    Highcharts.chart('two_week_about_book', {
                ...
                }, 
                {
                    name: '文字書比例',
                    type: 'spline',
                    data: two_weeks_words_book_rate,
                    tooltip: {
                        valueSuffix: '%'
                    }
                }
    ...
    series: [{
        ...
        {
            name: '繪本',
            y: about_picture_book/total
        },{
      
            name: '橋梁書',
            y: about_bridge_book/total
            
        }, {      
            name: '文字書',
            y: about_words_book/total
        }
            ]
    }]
});
            ...
            //     colors: ['#ACD6FF','#D9B3B3', '#CDCD9A ', '#A3D1D1',  '#C7C7E2', '#D2A2CC', '#d3a4ff', 
            //     '#EAC100', '#FF9224'],
            //     chart: {
