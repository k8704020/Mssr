<?php
//-------------------------------------------------------
//函式: numrow_book_read_frequency()
//用途: 學生閱讀次數
//日期: 2013年08月20日
//作者: mssr_team@cl_ncu
//-------------------------------------------------------

    function numrow_book_read_frequency($conn='',$user_id='',$date='',$array_filter=array(),$semester_start='',$semester_end='',$arry_conn){
    //---------------------------------------------------
    //函式: numrow_book_read_frequency()
    //用途: 學生閱讀次數
    //---------------------------------------------------
    ...
    //---------------------------------------------------
        ...
        if(!isset($user_id)||(trim($user_id)==='')){
            $err='NUMROW_BOOK_READ_FREQUENCY:NO USER_ID';
            die($err);
        }else{
            $user_id=(int)$user_id;
            if($user_id===0){
                $err='NUMROW_BOOK_READ_FREQUENCY:USER_ID IS INVAILD';
                die($err);
            }
        }
		...
		if(!is_array($array_filter)){
			$err='NUMROW_BOOK_READ_FREQUENCY:ARRAY_FILTER IS INVAILD';
            die($err);
		}else{
        ...
        if(!$conn){
            ...
            try{
                ...
                $err ='NUMROW_BOOK_READ_FREQUENCY:CONNECT FAIL';
                die($err);
            }
        }else{
            $has_conn=false;
        ...
        //資料庫
        $err='NUMROW_BOOK_READ_FREQUENCY:QUERY FAIL';
