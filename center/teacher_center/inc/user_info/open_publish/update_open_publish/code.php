...
//-------------------------------------------------------
    ...
    function update_open_publish($db_type='mysql',$arry_conn,$APP_ROOT,$user_id){
    //---------------------------------------------------
    //函式: update_open_publish()
    //用途: 更新上架條件
    //---------------------------------------------------
    ...
    //---------------------------------------------------
        ...
        //-----------------------------------------------
            ...
            if(!isset($arry_conn)||(empty($arry_conn))){
                $err='UPDATE_OPEN_PUBLISH:NO ARRY_CONN';
                die($err);
            }

            if(!isset($APP_ROOT)||trim($APP_ROOT)===''){
                $err='UPDATE_OPEN_PUBLISH:NO APP_ROOT';
                die($err);
            }

            if(!isset($user_id)||(trim($user_id)==='')){
                $err='UPDATE_OPEN_PUBLISH:NO USER_ID';
                die($err);
            }else{
                $user_id=(int)$user_id;
                if($user_id===0){
                    $err='UPDATE_OPEN_PUBLISH:USER_ID IS INVAILD';
                    die($err);
                }
            }
        ...
        //-----------------------------------------------
            ...
            try{
                ...
                $err ='UPDATE_OPEN_PUBLISH:CONNECT FAIL';
                die($err);
            }
        ...
        //-----------------------------------------------
            ...
            $err='UPDATE_OPEN_PUBLISH:QUERY FAIL';
            ...
            //有資料存在
                ...
                if((!empty($auth))&&(isset($auth['open_publish']))){
                    $open_publish=(int)$auth['open_publish'];
                ...
                //回填上架條件
                    ...
                    ";
                    //送出
                    $err ='UPDATE_OPEN_PUBLISH:DB QUERY FAIL';
            ...
            //新增預設權限資料
                ...
                $edit_by     =(int)$user_id;
                $user_id     =(int)$user_id;
                $auth        ='a:1:{s:12:"open_publish";i:1;}';
                $keyin_cdate ='NOW()';
                $keyin_mdate ='NULL';
                $keyin_ip    =get_ip();
