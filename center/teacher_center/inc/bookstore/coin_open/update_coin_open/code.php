...
//-------------------------------------------------------
    ...
    function update_coin_open($db_type='mysql',$arry_conn,$APP_ROOT,$user_id){
    //---------------------------------------------------
    //函式: update_coin_open()
    //用途: 更新葵幣開放條件
    //---------------------------------------------------
    ...
    //---------------------------------------------------
        ...
        //-----------------------------------------------
            ...
            if(!isset($arry_conn)||(empty($arry_conn))){
                $err='UPDATE_COIN_OPEN:NO ARRY_CONN';
                die($err);
            }

            if(!isset($APP_ROOT)||trim($APP_ROOT)===''){
                $err='UPDATE_COIN_OPEN:NO APP_ROOT';
                die($err);
            }

            if(!isset($user_id)||(trim($user_id)==='')){
                $err='UPDATE_COIN_OPEN:NO USER_ID';
                die($err);
            }else{
                $user_id=(int)$user_id;
                if($user_id===0){
                    $err='UPDATE_COIN_OPEN:USER_ID IS INVAILD';
                    die($err);
                }
            }
        ...
        //-----------------------------------------------
            ...
            try{
                ...
                $err ='UPDATE_COIN_OPEN:CONNECT FAIL';
                die($err);
            }
        ...
        //-----------------------------------------------
            ...
            $err='UPDATE_COIN_OPEN:QUERY FAIL';
            ...
            //有資料存在
                ...
                if((!empty($auth))&&(isset($auth['coin_open']))){
                    $coin_open=trim($auth['coin_open']);
                ...
                //回填葵幣開放條件
                    ...
                    ";
                    //送出
                    $err ='UPDATE_COIN_OPEN:DB QUERY FAIL';
            ...
            //新增預設權限資料
                ...
                $edit_by     =(int)$user_id;
                $user_id     =(int)$user_id;
                $auth        ='a:1:{s:9:"coin_open";s:3:"yes";}';
                $keyin_cdate ='NOW()';
                $keyin_mdate ='NULL';
                $keyin_ip    =get_ip();
