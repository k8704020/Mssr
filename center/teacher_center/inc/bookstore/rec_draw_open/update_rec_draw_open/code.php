...
//-------------------------------------------------------
    ...
    function update_rec_draw_open($db_type='mysql',$arry_conn,$APP_ROOT,$user_id){
    //---------------------------------------------------
    //函式: update_rec_draw_open()
    //用途: 更新推薦畫圖開放條件
    //---------------------------------------------------
    ...
    //---------------------------------------------------
        ...
        //-----------------------------------------------
            ...
            if(!isset($arry_conn)||(empty($arry_conn))){
                $err='UPDATE_REC_DRAW_OPEN:NO ARRY_CONN';
                die($err);
            }

            if(!isset($APP_ROOT)||trim($APP_ROOT)===''){
                $err='UPDATE_REC_DRAW_OPEN:NO APP_ROOT';
                die($err);
            }

            if(!isset($user_id)||(trim($user_id)==='')){
                $err='UPDATE_REC_DRAW_OPEN:NO USER_ID';
                die($err);
            }else{
                $user_id=(int)$user_id;
                if($user_id===0){
                    $err='UPDATE_REC_DRAW_OPEN:USER_ID IS INVAILD';
                    die($err);
                }
            }
        ...
        //-----------------------------------------------
            ...
            try{
                ...
                $err ='UPDATE_REC_DRAW_OPEN:CONNECT FAIL';
                die($err);
            }
        ...
        //-----------------------------------------------
            ...
            $err='UPDATE_REC_DRAW_OPEN:QUERY FAIL';
            ...
            //有資料存在
                ...
                if((!empty($auth))&&(isset($auth['rec_draw_open']))){
                    $rec_draw_open=trim($auth['rec_draw_open']);
                    ...
                    //-----------------------------------
                        ...
                        return $rec_draw_open;
                }else{
                //回填推薦畫圖開放條件
                    $auth['rec_draw_open']='yes';
                    $auth=serialize($auth);
                    ...
                    ";
                    //送出
                    $err ='UPDATE_REC_DRAW_OPEN:DB QUERY FAIL';
            ...
            //新增預設權限資料
                ...
                $edit_by     =(int)$user_id;
                $user_id     =(int)$user_id;
                $auth        ='a:1:{s:13:"rec_draw_open";s:3:"yes";}';
                $keyin_cdate ='NOW()';
                $keyin_mdate ='NULL';
                $keyin_ip    =get_ip();
