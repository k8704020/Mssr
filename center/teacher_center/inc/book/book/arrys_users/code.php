        ...
        if($date!==''){
            $sql="
                ...
                WHERE 1=1
                    ...
                    AND `end`  >='{$date}'
            ";
            $arrys_semester_result=$conn->query($sql);
            if(($arrys_semester_result->rowCount())!==0){
                $arry_row      =$arrys_semester_result->fetch(PDO::FETCH_ASSOC);
                $semester_start=trim($arry_row['start']);
                $semester_end  =trim($arry_row['end']);
                $sql="
                    SELECT `uid`
                    FROM `student`
                    WHERE 1=1
