...
//-------------------------------------------------------
    ...
    //---------------------------------------------------
        ...
        if(isset($_SESSION['a']['query']['m_forum_active_analysis']['filter'])){
            $filter=trim($_SESSION['a']['query']['m_forum_active_analysis']['filter']);
        }
        if(isset($_SESSION['a']['query']['m_forum_active_analysis']['query_fields'])){
            $query_fields=$_SESSION['a']['query']['m_forum_active_analysis']['query_fields'];
        }
        //echo "<Pre>";
        //print_r($filter);
        //echo "</Pre>";
...
//-------------------------------------------------------
    ...
    //---------------------------------------------------
        ...
        $query_year=(isset($_SESSION['a']['query']['m_forum_active_analysis']['query_year']))?(int)$_SESSION['a']['query']['m_forum_active_analysis']['query_year']:(int)date("Y");
                ...
                SELECT COUNT(*) AS `cno` FROM (
                    ...
                    FROM `user`.`student`
                        ...
                        INNER JOIN (
                            ...
                            WHERE 1=1
                                AND `mssr_forum`.`mssr_forum_article`.`keyin_cdate` BETWEEN '{$query_start}' AND '{$query_end}'
