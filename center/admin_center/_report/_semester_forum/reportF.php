...
<div id="container">
    ...
    <table id="mod_data_tbl" cellpadding="0" cellspacing="0" border="1" width="100%" style="font-size:20px;" rules='rows'/>
        ...
        <tr align="center">
            ...
            <td width="120px" height="40px">學生人數</td>
            <td width="120px" height="40px">好友數量</td>
            <td width="120px" height="40px">發表文章數</td>
            <td width="120px" height="40px">回應文章數</td>
            <td width="120px" height="40px">互動次數</td>
            <td width="120px" height="40px">參與小組數</td>
            <td width="120px" height="40px">導師好友數量</td>
            <td width="120px" height="40px">導師發表文章數</td>
            <td width="120px" height="40px">導師回應文章數</td>
            <td width="120px" height="40px">導師互動次數</td>
            <td width="120px" height="40px">導師參與小組數</td>
        </tr>
        ...
        //-----------------------------------------------
            ...
            if($users!==''){
                $sql="
                    ...
                    WHERE 1=1
                        AND
                        (   `mssr_forum`.`mssr_forum_friend`.`user_id` IN ({$users})
                            OR
                            `mssr_forum`.`mssr_forum_friend`.`friend_id` IN ({$users})
                        )
                ...
                $friend_cno=(int)$db_results[0]['cno'];
            }
        ...
        //-----------------------------------------------
            ...
            if($users!==''){
                $sql="
                    ...
                    WHERE 1=1
                        AND `mssr_forum`.`mssr_forum_article`.`user_id` IN ({$users})
                        AND `mssr_forum`.`mssr_forum_article`.`keyin_cdate` BETWEEN '{$rs_semester_start}' AND '{$rs_semester_end}'
                        AND `mssr_forum`.`mssr_forum_article`.`article_state`=1
                ";
                $db_results=db_result($conn_type='pdo',$conn_mssr,$sql,$arry_limit=array(),$arry_conn_mssr);
                $s_article_cno=(int)$db_results[0]['cno'];
            }
        ...
        //-----------------------------------------------
            ...
            if($users!==''){
                $sql="
                    ...
                    WHERE 1=1
                        AND `mssr_forum`.`mssr_forum_reply`.`user_id` IN ({$users})
                        AND `mssr_forum`.`mssr_forum_reply`.`keyin_cdate` BETWEEN '{$rs_semester_start}' AND '{$rs_semester_end}'
                        AND `mssr_forum`.`mssr_forum_reply`.`reply_state`=1
                ";
                $db_results=db_result($conn_type='pdo',$conn_mssr,$sql,$arry_limit=array(),$arry_conn_mssr);
                $s_reply_cno=(int)$db_results[0]['cno'];
            }
        ...
        //-----------------------------------------------
            ...
            if($users!==''){
                $sql="
                    ...
                    WHERE 1=1
                        AND `mssr_forum`.`mssr_forum_group_user_rev`.`user_id` IN ({$users})
                        AND `mssr_forum`.`mssr_forum_group_user_rev`.`user_state` =1
                        AND `mssr_forum`.`mssr_forum_group_user_rev`.`keyin_cdate` BETWEEN '{$rs_semester_start}' AND '{$rs_semester_end}'
                    GROUP BY `mssr_forum`.`mssr_forum_group_user_rev`.`group_id`
                ";
                $db_results=db_result($conn_type='pdo',$conn_mssr,$sql,$arry_limit=array(),$arry_conn_mssr);
                $s_group_cno=count($db_results);
            }
        ...
        //-----------------------------------------------
            ...
            if($users!==''){
                $sql="
                    ...
                    WHERE 1=1
                        AND
                        (   `mssr_forum`.`mssr_forum_friend`.`user_id` IN ({$rs_uid})
                            OR
                            `mssr_forum`.`mssr_forum_friend`.`friend_id` IN ({$rs_uid})
                        )
                        AND `mssr_forum`.`mssr_forum_friend`.`friend_state`=1
                ";
                $db_results=db_result($conn_type='pdo',$conn_mssr,$sql,$arry_limit=array(),$arry_conn_mssr);
                $t_friend_cno=(int)$db_results[0]['cno'];
            }
        ...
        //-----------------------------------------------
            ...
            if($users!==''){
                $sql="
                    ...
                    FROM `mssr_forum`.`mssr_forum_article`
                    WHERE 1=1
                        AND `mssr_forum`.`mssr_forum_article`.`user_id` IN ({$rs_uid})
                        AND `mssr_forum`.`mssr_forum_article`.`keyin_cdate` BETWEEN '{$rs_semester_start}' AND '{$rs_semester_end}'
                        AND `mssr_forum`.`mssr_forum_article`.`article_state`=1
                ";
                $db_results=db_result($conn_type='pdo',$conn_mssr,$sql,$arry_limit=array(),$arry_conn_mssr);
                $t_article_cno=(int)$db_results[0]['cno'];
            }
        ...
        //-----------------------------------------------
            ...
            if($users!==''){
                $sql="
                    ...
                    FROM `mssr_forum`.`mssr_forum_reply`
                    WHERE 1=1
                        AND `mssr_forum`.`mssr_forum_reply`.`user_id` IN ({$rs_uid})
                        AND `mssr_forum`.`mssr_forum_reply`.`keyin_cdate` BETWEEN '{$rs_semester_start}' AND '{$rs_semester_end}'
                        AND `mssr_forum`.`mssr_forum_reply`.`reply_state`=1
                ";
                $db_results=db_result($conn_type='pdo',$conn_mssr,$sql,$arry_limit=array(),$arry_conn_mssr);
                $t_reply_cno=(int)$db_results[0]['cno'];
            }
        ...
        //-----------------------------------------------
            ...
            if($users!==''){
                $sql="
                    ...
                    FROM `mssr_forum`.`mssr_forum_group_user_rev`
                    WHERE 1=1
                        AND `mssr_forum`.`mssr_forum_group_user_rev`.`user_id` IN ({$rs_uid})
                        AND `mssr_forum`.`mssr_forum_group_user_rev`.`user_state` =1
                        AND `mssr_forum`.`mssr_forum_group_user_rev`.`keyin_cdate` BETWEEN '{$rs_semester_start}' AND '{$rs_semester_end}'
                    GROUP BY `mssr_forum`.`mssr_forum_group_user_rev`.`group_id`
                ";
                $db_results=db_result($conn_type='pdo',$conn_mssr,$sql,$arry_limit=array(),$arry_conn_mssr);
                $t_group_cno=count($db_results);
            }
        ...
        //-----------------------------------------------
            ...
            if($users!==''){
                $sql="
                    ...
                    WHERE 1=1
                        AND (
                            `mssr_forum`.`mssr_forum_user_request`.`request_from` IN ({$users})
                            OR
                            `mssr_forum`.`mssr_forum_user_request`.`request_to` IN ({$users})
                        )
                        AND `mssr_forum`.`mssr_forum_user_request`.`keyin_cdate` BETWEEN '{$rs_semester_start}' AND '{$rs_semester_end}'
                ";
                $db_results=db_result($conn_type='pdo',$conn_mssr,$sql,$arry_limit=array(),$arry_conn_mssr);
                $s_request_cno=count($db_results);
            }
        ...
        //-----------------------------------------------
            ...
            if($users!==''){
                $sql="
                    ...
                    WHERE 1=1
                        AND (
                            `mssr_forum`.`mssr_forum_user_request`.`request_from` IN ({$rs_uid})
                            OR
                            `mssr_forum`.`mssr_forum_user_request`.`request_to` IN ({$rs_uid})
                        )
                        AND `mssr_forum`.`mssr_forum_user_request`.`keyin_cdate` BETWEEN '{$rs_semester_start}' AND '{$rs_semester_end}'
                ";
                $db_results=db_result($conn_type='pdo',$conn_mssr,$sql,$arry_limit=array(),$arry_conn_mssr);
                $t_request_cno=count($db_results);
            }
        ?>
        <tr align="center">
            ...
            <td width="120px" height="40px"><?php echo ($friend_cno);?>     </td>
            <td width="120px" height="40px"><?php echo ($s_article_cno);?>  </td>
            <td width="120px" height="40px"><?php echo ($s_reply_cno);?>    </td>
            <td width="120px" height="40px"><?php echo ($s_request_cno);?>  </td>
            <td width="120px" height="40px"><?php echo ($s_group_cno);?>    </td>
            <td width="120px" height="40px"><?php echo ($t_friend_cno);?>   </td>
            <td width="120px" height="40px"><?php echo ($t_article_cno);?>  </td>
            <td width="120px" height="40px"><?php echo ($t_reply_cno);?>    </td>
            <td width="120px" height="40px"><?php echo ($t_request_cno);?>  </td>
            <td width="120px" height="40px"><?php echo ($t_group_cno);?>    </td>
        </tr>
        <?php endforeach;?>
    </table>
