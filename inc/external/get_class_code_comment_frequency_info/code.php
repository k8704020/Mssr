<?php
//-------------------------------------------------------
//函式: get_class_code_comment_frequency_info()
//用途: 取得班級平均指導次數
//日期: 2014年11月30日
//作者: mssr_team@cl_ncu
//-------------------------------------------------------
    ...
    //---------------------------------------------------
        ...
        function get_class_code_comment_frequency_info($class_code,$arry_time){
        //-----------------------------------------------
        //函式: get_class_code_comment_frequency_info()
        //用途: 取得班級平均指導次數
        //-----------------------------------------------
        ...
        //-----------------------------------------------
            ...
            //$arry_time    時間條件
                ...
                if(!isset($class_code)||(trim($class_code)==='')){
                    $err='GET_CLASS_CODE_COMMENT_FREQUENCY_INFO:NO CLASS_CODE';
                    die($err);
                }else{
                ...
                if(!isset($arry_time)||(!is_array($arry_time))||(empty($arry_time))){
                    $err='GET_CLASS_CODE_COMMENT_FREQUENCY_INFO:NO ARRY_TIME';
                    die($err);
                }else{
                ...
                try{
                    ...
                    $err ='GET_CLASS_CODE_COMMENT_FREQUENCY_INFO:CONNECT FAIL';
                    die($err);
                }
            ...
            //$arry_time    時間條件
                ...
                $arry_time =$arry_time;
                $arry_class_code_comment_frequency=array();
            ...
            //-------------------------------------------
                ...
                //---------------------------------------
                    ...
                    if(empty($db_results)){
                        $err='GET_CLASS_CODE_COMMENT_FREQUENCY_INFO:CLASS_CODE ERROR';
                        die($err);
                    }
                ...
                //---------------------------------------
                    ...
                    if(empty($db_results)){
                        foreach($arry_time as $time){
                            $time =trim($time);
                            $arry_class_code_comment_frequency[$time]=0;
                        }
                        //回傳
                        return $arry_class_code_comment_frequency;
                    }else{
                ...
                //---------------------------------------
                    ...
                    foreach($arry_time as $time){
                        ...
                        $sql="
                            SELECT
                                `mssr`.`mssr_rec_comment_log`.`user_id`,
                                `mssr`.`mssr_rec_comment_log`.`keyin_cdate`
                            FROM  `mssr`.`mssr_rec_comment_log`
                            WHERE 1=1
                                AND DATE(`mssr`.`mssr_rec_comment_log`.`keyin_cdate`) BETWEEN '{$time}-1 00:00:00' AND '{$time}-{$days} 23:59:59'
                                AND `mssr`.`mssr_rec_comment_log`.`user_id` IN ({$uid_list})
                        ";
                        $db_results=db_result($conn_type='pdo',$conn_mssr,$sql,$arry_limit=array(),$arry_conn_mssr);
                        if(empty($db_results)){
                            $class_code_comment_frequency=0;
                        }else{
                            $total_comment_frequency        =count($db_results);
                            $class_code_comment_frequency   =ceil((int)$total_comment_frequency/(int)count($arry_uid));
                        }
                        $arry_class_code_comment_frequency[$time]=$class_code_comment_frequency;
                    }
