<?php
//-------------------------------------------------------
//函式: get_class_code_rec_group_info()
//用途: 取得班級平均推薦本數
//日期: 2014年11月30日
//作者: mssr_team@cl_ncu
//-------------------------------------------------------
    ...
    //---------------------------------------------------
        ...
        function get_class_code_rec_group_info($class_code,$arry_time){
        //-----------------------------------------------
        //函式: get_class_code_rec_group_info()
        //用途: 取得班級平均推薦本數
        //-----------------------------------------------
        ...
        //-----------------------------------------------
            ...
            //$arry_time    時間條件
                ...
                if(!isset($class_code)||(trim($class_code)==='')){
                    $err='GET_CLASS_CODE_REC_GROUP_INFO:NO CLASS_CODE';
                    die($err);
                }else{
                ...
                if(!isset($arry_time)||(!is_array($arry_time))||(empty($arry_time))){
                    $err='GET_CLASS_CODE_REC_GROUP_INFO:NO ARRY_TIME';
                    die($err);
                }else{
                ...
                try{
                    ...
                    $err ='GET_CLASS_CODE_REC_GROUP_INFO:CONNECT FAIL';
                    die($err);
                }
            ...
            //$arry_time    時間條件
                ...
                $arry_time =$arry_time;
                $arry_class_code_rec_group=array();
            ...
            //-------------------------------------------
                ...
                //---------------------------------------
                    ...
                    if(empty($db_results)){
                        $err='GET_CLASS_CODE_REC_GROUP_INFO:CLASS_CODE ERROR';
                        die($err);
                    }
                ...
                //---------------------------------------
                    ...
                    if(empty($db_results)){
                        foreach($arry_time as $time){
                            $arry_class_code_rec_group[$time]=0;
                        }
                        //回傳
                        return $arry_class_code_rec_group;
                    }else{
                ...
                //---------------------------------------
                    ...
                    foreach($arry_time as $time){
                        ...
                        $sql="
                            ...
                            WHERE 1=1
                                AND `mssr`.`mssr_rec_book_cno`.`keyin_mdate` BETWEEN '{$time}-1 00:00:00' AND '{$time}-{$days} 23:59:59'
                                AND `mssr`.`mssr_rec_book_cno`.`user_id` IN ({$uid_list})
                        ...
                        if(empty($db_results)){
                            $class_code_rec_group   =0;
                        }else{
                            $total_rec_group        =count($db_results);
                            $class_code_rec_group   =ceil((int)$total_rec_group/(int)count($arry_uid));
                        }
                        $arry_class_code_rec_group[$time]=$class_code_rec_group;
                    }
