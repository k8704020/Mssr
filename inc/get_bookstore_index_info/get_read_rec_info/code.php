	...
	function LHAS_read_info_record_interval($user_id,$sdate,$edate)
	{
		...
		$sql = "
			SELECT 
				...
				`mssr_idc_book_sticker_level_info`.`administrator_level`,
				`mssr_idc_book_sticker_level_info`.`pages`,
				`mssr_idc_book_sticker_level_info`.`words`
			FROM `mssr_book_borrow_log`
			INNER JOIN `mssr_idc_book_sticker_level_info`
					ON `mssr_book_borrow_log`.`book_sid` = `mssr_idc_book_sticker_level_info`.`book_sid`
			WHERE `mssr_book_borrow_log`.`user_id` = '{$user_id}' 
				AND `mssr_book_borrow_log`.`borrow_sdate` >= '{$sdate}' 
				AND SUBSTR(`mssr_book_borrow_log`.`borrow_sdate`,1,10) <= '{$edate}'
			ORDER BY `mssr_book_borrow_log`.`borrow_sdate` ASC
		 ";
		$result = db_result($conn_type='pdo',$conn_mssr,$sql,$arry_limit=array(),$arry_conn_mssr);
		...
		if (!empty($result)) {
			foreach($result as $key=>$val){
				...
				$data[$key]["book_sid"] = $val["book_sid"];
				$data[$key]["time"] = $val["borrow_sdate"];
				$data[$key]["administrator_level"] = $val["administrator_level"];
				$data[$key]["pages"] = $val["pages"];
				$data[$key]["words"] = $val["words"];
				$get_book_read_opinion_log_info=get_book_read_opinion_log_info($conn_mssr,$val["borrow_sid"],$array_filter=array('opinion_answer'),$arry_conn_mssr);
							...
							if($topic_id===1){
								...
								if(unserialize($topic_options)){
									...
									if(isset($arrys_topic_options[$read_num])){
										$data[$key]["read_num"]=trim($arrys_topic_options[$read_num]);
									}
								}
							}
							if($topic_id===2){
								...
								if(unserialize($topic_options)){
									$arrys_topic_options=unserialize($topic_options);
									if(isset($arrys_topic_options[$read_num])){
										$data[$key]["diffcult"]=trim($arrys_topic_options[$read_num]);
									}
								}
							}
							if($topic_id===5){
								...
								if(unserialize($topic_options)){
									...
									if(isset($arrys_topic_options[$like_num])){
										$data[$key]["like_num"]=trim($arrys_topic_options[$like_num]);
									}
								}
							}
						}
	...
	function LHAS_read_info_record_limit($user_id,$sdate,$semesterStart,$limit)
	{
		...
		$sql = "
			...
			INNER JOIN `mssr_idc_book_sticker_level_info`
					ON `mssr_book_borrow_log`.`book_sid` = `mssr_idc_book_sticker_level_info`.`book_sid`
			WHERE `mssr_book_borrow_log`.`user_id` = '{$user_id}' 
				AND `mssr_book_borrow_log`.`borrow_sdate` >= '{$semesterStart}' 
				AND SUBSTR(`mssr_book_borrow_log`.`borrow_sdate`,1,10) <= '{$sdate}'
			ORDER BY `mssr_book_borrow_log`.`borrow_sdate` DESC limit 0,{$limit}
		";
		$result = db_result($conn_type='pdo',$conn_mssr,$sql,$arry_limit=array(),$arry_conn_mssr);
		...
		$sql = "
			...
			FROM (
				...
				INNER JOIN `mssr_idc_book_sticker_level_info`
					ON `mssr_book_borrow_log`.`book_sid` = `mssr_idc_book_sticker_level_info`.`book_sid`
				WHERE `mssr_book_borrow_log`.`user_id` = '{$user_id}' 
					AND `mssr_book_borrow_log`.`borrow_sdate`>= '{$start}' 
					AND SUBSTR(`mssr_book_borrow_log`.`borrow_sdate`,1,10) <= '{$end}'
				ORDER BY `mssr_book_borrow_log`.`borrow_sdate` DESC
			) as t1
