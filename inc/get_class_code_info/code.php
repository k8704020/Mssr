...
//-------------------------------------------------------
    ...
    function get_class_code_info($conn='',$school_code='',$grade=0,$compile_class_code_name=false,$arry_conn){
    //---------------------------------------------------
    //函式: get_class_code_info()
    //用途: 提取班級資訊
    //---------------------------------------------------
    ...
    //---------------------------------------------------
        ...
        if(!isset($compile_class_code_name)){
            $compile_class_code_name=false;
        }else{
            $compile_class_code_name=$compile_class_code_name;
        }

        if((!$arry_conn)||(empty($arry_conn))){
            $err='GET_CLASS_CODE_INFO:NO ARRY_CONN';
            die($err);
        }
        ...
        if(!$conn){
            ...
            try{
                ...
                $err ='GET_CLASS_CODE_INFO:CONNECT FAIL';
                die($err);
            }
        }else{
            $has_conn=false;
        ...
        $sql.="
            ...
            WHERE 1=1
                AND '{$date}' BETWEEN DATE(`semester`.`start`) AND DATE(`semester`.`end`)
        ...
        //資料庫
        $err='GET_CLASS_CODE_INFO:QUERY FAIL';
